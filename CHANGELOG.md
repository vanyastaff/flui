...
- **Cross-thread signal writes are routed by the slot's graph** (`flui-app`, `flui-view`,
  ADR-0085 §1): the internal `SignalWrite` command runs against the graph of the presentation
  that minted the slot instead of always the primary's, where a write to a secondary window's
  signal would have failed with `ForeignGraph`. The defect was latent: the command has no public
