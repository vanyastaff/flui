...
| `crossbeam-channel` 0.5 | `std::sync::mpsc::sync_channel(0)` | `flui-app/ui_realm.rs`, `flui-engine/raster_owner.rs`, `flui-platform/winit/control.rs` | Используются только `bounded(0)` (rendezvous), `try_send`, `recv_timeout` — ни `select!`, ни клонирования `Receiver`. Всё это есть в std, а std-`mpsc` с 1.67 **и есть** crossbeam-реализация. Минус одна зависимость без изменения семантики. Когда стабилизируется `std::sync::mpmc` — тем более |
| `regex` 1 в `flui-engine/build.rs` | `regex-lite` или ручной парсер | `build.rs:57`, 3 `Regex::new` | `regex` + `regex-syntax` + `aho-corasick` компилируются в каждой холодной сборке ради трёх шаблонов в build-скрипте; `regex-lite` — один крейт без зависимостей |
...
| следующие 2–3 спринта | `cocoa`/`objc` → `objc2`/`objc2-app-kit` в macOS backend; `crossbeam-channel` → `std::sync::mpsc`; `regex` → `regex-lite` в `flui-engine/build.rs`; `Rodeo` → `ThreadedRodeo` (§6b) | Deprecated-зависимость; минус три зависимости без изменения семантики |
...
- Stable-baseline времени сборки не снят: локальный toolchain `1.98.0` установлен частично (rustup при `toolchain list` попытался «recover from a partially installed toolchain» и не смог; `cargo check` падает с `can't find crate for std`). Стоит переустановить: `rustup toolchain uninstall 1.98.0 && rustup toolchain install 1.98.0`.
