...
| **S6** — Recursive widgets need derive-hint | 1-frame (pain) — single but high-incidence | FR-009 correction note, Edge Cases (Recursive widgets), `crates/flui-macros/README.md` | **Round-5 revision + 2026-06-30 correction**: derive-attribute mechanism (`#[view(recursive)]`) dropped per doc-review (proc-macro infeasibility + FR-010 two-paths violation). Impl-level `#[diagnostic::on_unimplemented]` is also impossible because rustc accepts it only on trait definitions. Canonical rule remains `.boxed()` at the recursion/branch edge; docs carry discovery for Phase 3. |
