...
| 5 | **CommandIR data-type lift** (`command_ir.rs`): move `DrawSegment`/`DrawItem`/`draw_order`/`SavedLayer`/`PendingOpacityLayer` **type-defs** out of painter.rs into `command_ir.rs`, baking semantics byte-identical (still stores lowered GPU bytes). Pure type-move + re-export. **Lands BEFORE the compositor** (task 8) so both it and the batcher import shared IR types from one home (no double-touch). | C5-partial, C1, C8 (snapshot unchanged — pure move), C9, C10, C11 | `chief-architect` | — | ◐ (building) |
