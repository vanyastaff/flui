...
| C2 | **Scroll physics + Scrollbar + RefreshIndicator**. | DONE v1 (merge `5192f136`) | ScrollController (Arc ChangeNotifier) + ScrollPhysics (Clamping/Bouncing) + Scrollable (pan→set_pixels) + Scrollbar. GestureDetector gained HitTestBehavior knob (default DeferToChild). 13 scroll tests; 176 widgets green. fling-sim DONE (merge `81366265` — momentum+bounce-back via AnimationController+VsyncScope; +4 NaN/INF core fixes found wiring it). DEFERRED: thumb-drag, nested, RefreshIndicator, snapping, keyboard. |
