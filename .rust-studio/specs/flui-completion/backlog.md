...
| C2 | **Scroll physics + Scrollbar + RefreshIndicator**. | DONE v1 (merge `5192f136`) | ScrollController (Arc ChangeNotifier) + ScrollPhysics (Clamping/Bouncing) + Scrollable (pan→set_pixels) + Scrollbar. GestureDetector gained HitTestBehavior knob (default DeferToChild). 13 scroll tests; 176 widgets green. fling-sim DONE (merge `81366265` — momentum+bounce-back via AnimationController+VsyncScope; +4 NaN/INF core fixes found wiring it). Scrollbar thumb-drag + RefreshIndicator (pull-to-refresh, sync on_refresh + RefreshController::finish) DONE (`26efa504`). DEFERRED: animated spinner (static v1), nested-scroll, snapping, keyboard, overscroll-glow. |
...
**BREAKTHROUGH 2026-06-27 — the whole project is green.** The `CARGO_INCREMENTAL=0` tools fix (`22f878b4`) was the keystone: it unblocked the GPU build (wgpu rustc-segfault) AND resolved the flui-platform "STATUS_HEAP_CORRUPTION" exclusion — BOTH were the same sccache+incremental crash, NOT code bugs. The DX crates (devtools/build/cli) were then fixed (missing deps/features) and brought into the workspace (`147452a3`). **`cargo nextest run --workspace` (NO exclusions) = 4869/4869 tests PASS** (14 skipped = GPU-device/image-feature). Every member crate — flui-platform, flui-engine, flui-app, devtools/build/cli, all of it — compiles + tests green. The earlier ~70% estimate was way low; the project is functionally near-complete with the entire suite green. Remaining = INCOMPLETE-not-broken: C1 lazy slivers (design-heavy), flui-reactivity (standalone by design, 0 consumers), the documented v1 deferrals (TextField IME/selection, scroll fling, …), runtime e2e (actual window — needs manual/example run), and a CI-config candidate (the flui-platform CI exclusion can likely be dropped now, but verify it's not intermittently flaky before un-excluding).
