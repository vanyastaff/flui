...
- **`cargo xtask workspace`**: the kind rule of
  ...
  `flui-sdk`, `flui-platform-api` and `flui-protocol`; each refused edge needs the dependent's
...
- **`flui-macros`**: the derives (`StatelessView`, `StatefulView`, `InheritedData`,
  `Diagnosticable`, `Animatable`) look up `flui-sdk` in the consumer's manifest first, then the
  owning crate, then `flui`, so a package that depends on `flui-sdk` alone can use them.
