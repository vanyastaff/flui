...
/// no `Send`/`Sync` bound in its own type. Cross-linked to
...
/// use flui_rendering::BoxConstraints;
...
/// // error[E0277]: `(dyn Fn(RenderId, BoxConstraints) -> Size + '_)`
/// // cannot be shared between threads safely -- `LayoutChildCallback`
/// // carries no `Send`/`Sync` bound, so a `RenderBox::perform_layout` body
/// // that tried to hand its layout-child callback to another thread never
/// // reaches a cross-thread `NodePtr` deref; it fails to compile instead
/// // of hitting the deleted `check_thread` panic at runtime.
/// std::thread::spawn(move || {
///     let _ = cb;
/// });
/// ```
