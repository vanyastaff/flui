...
/// window itself, and the close-request handler plus frame-failure detail
/// policy its caller's `AppConfig` carried. The detail policy is consumed
/// only when `SeparateRealms` constructs a new realm; `SharedRealm` inherits
/// the already-hosted realm's policy.
#[cfg(all(
    not(target_os = "android"),
    not(target_os = "ios"),
    not(target_arch = "wasm32")
...
pub(super) fn drain_pending_secondary_window_completions() {
    ...
    for (policy, window, close_request_handler, frame_failure_detail) in pending {
        if let Err(error) = finish_open_secondary_window(
            policy,
            window,
...
> {
    ...
    match open {
        WindowOpen::Ready(window) => finish_open_secondary_window(
            policy,
            window,
            config.close_request_handler.clone(),
            config.frame_failure_detail,
        )
        .map(Some),
...
fn finish_open_secondary_window(
    ...
    let realm_dispatch = match policy {
        ...
        WindowPolicy::SeparateRealms => {
            ...
            // Deliberately NOT wired to a frame-failure handler in this
            // slice: only the detail policy is threaded through. This
            // secondary realm's failures therefore surface through
            // `tracing` alone until the report-side routing grows an
            // explicit secondary-realm handler contract.
