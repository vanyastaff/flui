...
//!
//! Loads a scene plugin shared library at runtime. Supports two symbol conventions:
//!
...
//! - **Scene plugin** (`flui_scene_build` / `flui_scene_version` / `flui_scene_drop`) —
//!   generated by [`scene_plugin!`], raw scene-building function
//!
//! The loader tries `flui_app_build` first and falls back to `flui_scene_build`.
//!
//! Cross-platform: uses [`DynLib`] for loading (dlopen on Unix, LoadLibraryW on Windows).
...
/// - `app_plugin!` symbols: `flui_app_build` / `flui_app_version` / `flui_app_drop`
/// - `scene_plugin!` symbols: `flui_scene_build` / `flui_scene_version` / `flui_scene_drop`
///
/// The loader tries `flui_app_build` first and falls back to `flui_scene_build`.
///
/// # Lifecycle
...
impl ScenePlugin {
    ...
    unsafe fn try_resolve(
        ...
        let drop_fn: Option<SceneDropFn> = lib.symbol(drop_sym).map(|ptr| std::mem::transmute(ptr));
    ...
    /// Check if the shared library file on disk has been modified since it was loaded.
