...
// Same boundary, same reason: `ResolvedFont` carries a `Family`, and a
// consumer that cannot name it cannot hold the result.
pub use cosmic_text::fontdb::Family;
pub use table_border::paint_table_border;
pub use text_layout::{
    LineInfo, ResolvedFont, SharedFontSystem, TextLayout, TextLayoutResult, detect_text_direction,
...
// facade — this canonical-home note is the single-source clarification
// (an explicit `as Paint` re-export alias was rejected: it adds no
