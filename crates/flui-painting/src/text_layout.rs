...
mod fallback {
    ...
    /// Stub text layout for use by `TextPainter` when the "text" feature is disabled.
    ///
    /// Provides estimated metrics based on character count rather than real shaping.
    #[derive(Debug)]
    pub struct TextLayout {
        /// Font size used for layout.
        font_size: f32,
    ...
    impl TextLayout {
        ...
        /// Returns the screen offset for a caret at the given text position (estimated).
        pub fn get_offset_for_caret(&mut self, position: TextPosition) -> Offset<Pixels> {
            // Estimate x position based on character index and average char width
        ...
        /// Returns the word boundary at the given text position (estimated).
        pub fn get_word_boundary(&self, position: TextPosition) -> TextRange {
            // Simple word boundary: find whitespace boundaries
            ...
            while start > 0 && bytes.get(start - 1).map_or(false, |&b| b != b' ') {
                start -= 1;
            }

            let mut end = offset;
            while end < bytes.len() && bytes.get(end).map_or(false, |&b| b != b' ') {
                end += 1;
            }
