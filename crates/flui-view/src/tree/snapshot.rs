...
pub trait ElementTreeNav: TreeNav<ElementId> {}

/// Blanket implementation for all TreeNav with ElementId.
impl<T: TreeNav<ElementId>> ElementTreeNav for T {}
...
pub struct SubtreeView<'a, T: TreeNav<ElementId>> {
    tree: &'a T,
    root: ElementId,
}

impl<'a, T: TreeNav<ElementId>> SubtreeView<'a, T> {
...
impl<T: TreeNav<ElementId>> std::fmt::Debug for SubtreeView<'_, T> {
...
pub struct DepthLimitedView<'a, T: TreeNav<ElementId>> {
    tree: &'a T,
    root: ElementId,
    max_depth: usize,
}

impl<'a, T: TreeNav<ElementId>> DepthLimitedView<'a, T> {
...
impl<T: TreeNav<ElementId>> std::fmt::Debug for DepthLimitedView<'_, T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
...
pub struct FilteredView<'a, T: TreeNav<ElementId>, P> {
    tree: &'a T,
    root: ElementId,
    predicate: P,
}

impl<'a, T: TreeNav<ElementId>, P> FilteredView<'a, T, P>
where
    P: Fn(ElementId) -> bool,
{
...
impl<T: TreeNav<ElementId>, P> std::fmt::Debug for FilteredView<'_, T, P> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
...
impl TreeSnapshot {
    ...
    pub fn from_tree<T: TreeNav<ElementId>>(tree: &T, root: ElementId) -> Self {
    ...
    fn capture_subtree<T: TreeNav<ElementId>>(&mut self, tree: &T, id: ElementId) {
...
impl<'a, T: TreeNav<ElementId>> AncestorView<'a, T> {
...
impl<T: TreeNav<ElementId>> std::fmt::Debug for AncestorView<'_, T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
...
impl<'a, T: TreeNav<ElementId>> SiblingView<'a, T> {
...
impl<T: TreeNav<ElementId>> std::fmt::Debug for SiblingView<'_, T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
...
pub trait TreeViewExt: TreeNav<ElementId> {
...
impl<T: TreeNav<ElementId>> TreeViewExt for T {}
