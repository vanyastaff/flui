...
- **A duplicate GlobalKey is reported at the frame boundary, as data** — every parent's keyed-child declaration is reserved for the frame (mount, graft, and in-place update alike); `BuildOwner::finalize_tree` verifies them, repairs the losing parent's dangling child edge, and appends a typed `DuplicateGlobalKey` to `take_global_key_diagnostics`. Reservations clear each frame, so a cross-frame reparent is not a duplicate. Do not turn this into a panic — the input is caller-controlled.
