...
impl<'a> EguiPainter<'a> {
    ...
    /// Helper: Interpolate color in gradient at position t (0.0 to 1.0)
    ///
    /// This is generic over gradient types that have colors and stops fields
    fn interpolate_gradient_color<T>(gradient: &T, t: f32) -> flui_types::styling::Color
    where
        T: GradientColors,
    {
        ...
        if colors.is_empty() {
            return flui_types::styling::Color::rgba(0, 0, 0, 0);
        }

        if colors.len() == 1 || stops.is_empty() {
            return colors[0];
        }
        ...
        if stop_index >= colors.len() - 1 {
            return *colors.last().unwrap();
        }

        let stop1 = stops[stop_index];
        let stop2 = stops[stop_index + 1];
        let color1 = colors[stop_index];
        let color2 = colors[stop_index + 1];

        // Linear interpolation
        let local_t = if stop2 - stop1 > 0.001 {
            (t - stop1) / (stop2 - stop1)
        } else {
            0.0
        };

        flui_types::styling::Color::rgba(
            (color1.r as f32 + (color2.r as f32 - color1.r as f32) * local_t) as u8,
            (color1.g as f32 + (color2.g as f32 - color1.g as f32) * local_t) as u8,
            (color1.b as f32 + (color2.b as f32 - color1.b as f32) * local_t) as u8,
            (color1.a as f32 + (color2.a as f32 - color1.a as f32) * local_t) as u8,
        )
    }
}

/// Helper trait for accessing gradient colors and stops
trait GradientColors {
    fn colors(&self) -> &[flui_types::styling::Color];
    fn stops(&self) -> &[f32];
}

impl GradientColors for flui_types::styling::LinearGradient {
    fn colors(&self) -> &[flui_types::styling::Color] {
        &self.colors
    }
    fn stops(&self) -> &[f32] {
        self.stops.as_ref().map(|s| s.as_slice()).unwrap_or(&[])
    }
}

impl GradientColors for flui_types::styling::RadialGradient {
    fn colors(&self) -> &[flui_types::styling::Color] {
        &self.colors
    }
    fn stops(&self) -> &[f32] {
...
impl GradientColors for flui_types::styling::SweepGradient {
    fn colors(&self) -> &[flui_types::styling::Color] {
        &self.colors
    }
    fn stops(&self) -> &[f32] {
...
impl<'a> Painter for EguiPainter<'a> {
    ...
    fn linear_gradient(&mut self, rect: Rect, gradient: &flui_types::styling::LinearGradient) {
        // egui doesn't have native gradient support yet, so we'll render multiple rects
        // to approximate the gradient. This is a simplified implementation.

        if gradient.colors.is_empty() {
            return;
        }

        // Number of steps to approximate gradient
        const GRADIENT_STEPS: usize = 32;

        let width = rect.width();
        let height = rect.height();
        let step_height = height / GRADIENT_STEPS as f32;

        for i in 0..GRADIENT_STEPS {
            let t = i as f32 / (GRADIENT_STEPS - 1) as f32;

            // Interpolate color based on stops
            let color = Self::interpolate_gradient_color(gradient, t);

            let y = rect.top() + i as f32 * step_height;
            let step_rect = Rect::from_xywh(rect.left(), y, width, step_height);
            ...
            let paint = Paint {
                ...
                stroke_width: 0.0,
                anti_alias: false, // Disable AA for gradient steps to avoid seams
            };

            self.rect(step_rect, &paint);
        }
    }
    ...
    fn radial_gradient(&mut self, rect: Rect, gradient: &flui_types::styling::RadialGradient) {
        ...
        // Draw from outside to inside to ensure proper layering
        for i in (0..GRADIENT_STEPS).rev() {
            let t = i as f32 / (GRADIENT_STEPS - 1) as f32;

            let color = Self::interpolate_gradient_color(gradient, t);
            let radius = max_radius * (i as f32 / (GRADIENT_STEPS - 1) as f32);
    ...
    fn sweep_gradient(&mut self, rect: Rect, gradient: &flui_types::styling::SweepGradient) {
        // Approximate sweep gradient with pie slices
        if gradient.colors.is_empty() {
            return;
        }
        ...
        let angle_step = std::f32::consts::TAU / GRADIENT_STEPS as f32;
