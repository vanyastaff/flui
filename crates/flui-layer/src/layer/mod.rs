...
//
// One `impl From<XxxLayer> for Layer` per variant, generated by
// `gen_layer_from_impls!` (see `dispatch.rs`). `boxed` marks the variants
// whose enum payload is `Box<T>`.

dispatch::gen_layer_from_impls! {
    Canvas => boxed CanvasLayer;
    Picture => boxed PictureLayer;
    ClipRect => ClipRectLayer;
    ClipRRect => ClipRRectLayer;
    ClipPath => boxed ClipPathLayer;
    ClipSuperellipse => ClipSuperellipseLayer;
    Offset => OffsetLayer;
    Transform => TransformLayer;
    Opacity => OpacityLayer;
    ...
    Follower => FollowerLayer;
    AnnotatedRegion => AnnotatedRegionLayer;
    PerformanceOverlay => boxed PerformanceOverlayLayer;
}
