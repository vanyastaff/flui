...
///
/// **This is the single source of truth** — `TreeNav::MAX_DEPTH`'s
/// default impl points here, and the visitor module's stack-sizing
/// constants (`MAX_STACK_DEPTH`, `STACK_SIZE`) are derived from this
/// value. Those defaults used to be defined independently and had
/// drifted to 32 / 64 / 48 across four call sites, which made
/// stack-frame budget reasoning inconsistent.
...
///
/// Chosen at 32 because typical widget / element / view trees rarely
/// exceed 32 levels (per Flutter's `Element.depth` survey); deeper
