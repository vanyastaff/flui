...
- **ADR-0028: no upward/sideways design-system dependency.** Runtime deps are only `flui-widgets`/`flui-view`/`flui-types`/`flui-animation` (plus `tracing`) — never `flui-material` (independent sibling design system) and never `flui-objects`/`flui-rendering` directly (v1 composes existing `flui-widgets` widgets; it paints nothing of its own). `flui-foundation`/`flui-interaction` are dev-dependencies only, for the `tests/common` mount harness (`ElementId`/`RenderId`/`Diagnosticable`, synthetic pointer events) — no `src/` code references either; there is no `FocusNode` wiring yet.
...
- **`CupertinoApp` (`app.rs`) composes downward only** — the iOS-style shell over `flui-widgets`' design-neutral `WidgetsApp` (ADR-0028/ADR-0042). Brightness follows Apple's model, not a `ThemeMode` enum: the theme's optional `brightness` override wins, otherwise the ambient `MediaQueryData::platform_brightness` drives resolution (`resolve_from` in the shell's own build, above the published `CupertinoTheme` — the oracle's materialization altitude, pinned by `tests/cupertino_app.rs`). Named deferrals (`CupertinoUserInterfaceLevel`, selection style, scroll behavior, `CupertinoLocalizations`) are in the module docs.
