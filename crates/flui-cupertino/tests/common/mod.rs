//! Headless mount harness shared by `flui-cupertino`'s integration tests — a
//! copy of `flui-material`'s `tests/common/mod.rs` (itself a copy of
//! `flui-widgets`' harness, extended with pointer dispatch and vsync-driven
//! ticking), reused verbatim so `CupertinoButton`'s tap/press-timing can be
//! driven at the widget level the same way `flui-material`'s `InkWell` is.
