...
| `routing` | Event dispatch infrastructure: `EventRouter`, `PointerRouter`, `FocusManager` (global singleton), `FocusScopeNode` / reading-order Tab traversal, `MouseTracker` (enter/exit/hover), hit testing, and the `TransformGuard` stack-RAII for the transform stack. Off the per-pointer hot path. |
...
| `FocusManager::global()` (`src/routing/focus.rs`) | `static OnceLock<FocusManager>` plus internal `parking_lot::RwLock` fields | Process-wide singleton | Off any hot path; Tab traversal reads the active `FocusScopeNode`, then writes primary focus once. |
