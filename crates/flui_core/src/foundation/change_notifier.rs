...
//! # Key Improvements
//!
//! - Uses `parking_lot::Mutex` instead of `std::sync::Mutex` (2-3x faster)
//! - Better performance under contention
//! - Smaller memory footprint

use parking_lot::Mutex; // ← IMPROVED: 2-3x faster than std::Mutex
...
impl ChangeNotifier {
    ...
    pub fn notify_listeners(&self) {
        let listeners = self.listeners.lock(); // ← parking_lot doesn't return Result
...
pub struct MergedListenable {
    #[allow(dead_code)] // TODO: Use this field to forward notifications from source listenables
