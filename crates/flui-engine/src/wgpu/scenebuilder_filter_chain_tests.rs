...
mod gpu_tests {
    ...
    fn acquire_test_device_and_queue() -> (Arc<wgpu::Device>, Arc<wgpu::Queue>) {
        crate::wgpu::test_support::test_device_and_queue("SceneBuilderFilterChain Test Device")
    }
    ...
    fn create_render_surface(device: &wgpu::Device) -> (wgpu::Texture, wgpu::TextureView) {
        crate::wgpu::test_support::create_sampleable_target(
            device,
            "SceneBuilderFilterChain Test Surface",
            SURFACE_WIDTH,
            SURFACE_HEIGHT,
            SURFACE_FORMAT,
        )
    ...
    fn clear_to_black(device: &wgpu::Device, queue: &wgpu::Queue, view: &wgpu::TextureView) {
        crate::wgpu::test_support::clear_target(device, queue, view, wgpu::Color::BLACK);
    }
