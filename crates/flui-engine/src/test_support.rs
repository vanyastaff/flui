//! Shared GPU scaffolding for the `testing` suites.
//!
...
/// [`crate::adapter::trusted_adapter_options`].
#[cfg(feature = "testing")]
...
pub(crate) fn try_test_device_and_queue(
    ...
    let device_and_queue = request_test_adapter()
    ...
    match device_and_queue {
        ...
        Err(reason) => {
            // Same rule as `renderer_or_skip`: a self-skipping test is
            // reported PASSED, so a GPU-demanding run fails loudly instead.
            resolve_unavailable_gpu(&reason, require_gpu());
            None
        }
    }
}
...
/// with `PollType::Wait`, and dumps the frame via [`crate::readback_dump`]
...
pub(crate) fn readback_bytes(
    ...
    crate::readback_dump::dump_frame(width, height, &bytes);
    bytes
}
...
pub(crate) fn renderer_or_skip() -> Option<crate::headless::HeadlessRenderer> {
    match pollster::block_on(crate::headless::HeadlessRenderer::new()) {
