...
pub(super) enum RestoreOutcome {
    ...
    Composite {
        ...
        /// [`crate::command_ir::PendingOpacityLayer`] without coupling the flush path.
...
impl LayerCompositor {
    ...
    pub(super) fn pop_layer(
        ...
        // Glyphs are a batch of the segment like any other, so a text-only
        // layer is non-empty here and reaches the Composite/Reintegrate
        // paths, where its text is composited WITH the layer's opacity/tint.
...
mod tests {
    ...
    /// A layer whose only content is TEXT must not resolve to `Empty`: the
    /// text IS the layer's content and must be composited with the layer's
    /// opacity, not dropped with its draw records.
    #[test]
    fn pop_layer_composites_a_text_only_layer() {
        ...
        // Text-only segment: one glyph quad, nothing else.
        let mut text_only = DrawSegment::new();
        let _ = text_only
            .glyph_batch
            .add(crate::instancing::GlyphInstance::new(
                [0.0, 0.0, 8.0, 12.0],
                [0, 0],
                false,
    ...
    fn pop_layer_composite_when_image_filter_set_even_at_full_opacity_white_tint() {
        use crate::command_ir::{ImageFilterSpec, MorphOp};
    ...
    fn pop_layer_composites_a_layer_a_clip_opened_even_with_no_sdf_to_apply() {
        ...
        compositor.push_layer(
            ...
            None,
            LayerFilterChain::new(),
            Some(crate::state_stack::ResolvedClip::NONE),
        );
