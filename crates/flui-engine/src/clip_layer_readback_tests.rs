...
/// swap gave up two things — it stopped intersecting under nesting (one SDF
/// slot, inner overwrites outer) and stopped being exact (its coarse scissor
/// is padded, so pixels leak up to a pixel out). See `Painter::clip_rect`.
///
...
fn a_clip_superellipse_layer_clips_to_the_squircle_not_its_bounding_rrect() {
    ...
    let render = |push_clip: &dyn Fn(&mut SceneBuilder, Clip)| {
...
/// `WgpuPainter` (4 KiB), a `LayerDispatcher` (4.2 KiB), and the encode
/// frames as live locals — and that cost is a per-frame constant:
...
fn a_deep_layer_chain_captures_without_overflowing_a_small_stack() {
    ...
    let pixels = std::thread::Builder::new()
        ...
        .spawn(move || {
            let mut tree = LayerTree::new(flui_layer::Layer::from(flui_layer::CanvasLayer::new()));
            let mut parent = tree.root();
            for _ in 1..DEPTH {
                parent = tree.push_child(
                    parent,
                    flui_layer::Layer::from(flui_layer::CanvasLayer::new()),
                );
            }
            renderer
                .render_layer_tree(&tree, (SIDE, SIDE))
