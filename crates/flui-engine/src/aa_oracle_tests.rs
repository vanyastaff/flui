...
mod gpu_tests {
    ...
    /// A non-uniform transform reaches the glyphs: under `scale(2, 1)` a
    /// label is twice as wide as at `scale(1, 1)` and no taller, so its ink
    /// is about half of what `scale(2, 2)` paints.
    ///
    /// Glyphs are rasterised at the CTM's larger axis and each quad carries
    /// the CTM's linear part over that scale (`DrawBatcher::draw_paragraph`),
    /// so the bitmap is resampled into the transformed shape. Before that,
    /// the run was placed at `max_scale` on both axes and this test pinned
    /// the ratio near 1 as a known limit.
    #[test]
    fn anisotropic_scale_squashes_glyphs_on_one_axis() {
        ...
        assert!(
            ...
            "scale(2,1) must paint about half the ink of scale(2,2); got \
             {wide_only}/{uniform_2x} = {ratio:.2} — near 1.0 means the run was \
             stretched uniformly by the larger axis again"
        );
    }
    ...
    fn a_rounded_clip_rounds_a_gradient() {
        ...
        painter.draw_rect(
            ...
            &crate::test_support::linear_gradient_fill(
                ...
                vec![Color::rgb(0, 0, 255), Color::rgb(0, 0, 255)],
            ),
        );
        painter.restore();
