...
pub(crate) struct CachedTexture {
    ...
    /// the shared `TextureAtlas` (`crate::atlas::TextureAtlas`) and should use these UVs
    /// when constructing a `TextureInstance` (`crate::instancing::TextureInstance`).
...
impl TextureCache {
    ...
    pub(crate) fn new(device: Arc<Device>, queue: Arc<Queue>) -> Self {
        let atlas = crate::atlas::TextureAtlas::new(
            &device,
            crate::atlas::ATLAS_DEFAULT_SIZE,
            crate::atlas::ATLAS_DEFAULT_SIZE,
            IMAGE_TEXTURE_FORMAT,
        );

        tracing::debug!(
            size = crate::atlas::ATLAS_DEFAULT_SIZE,
            threshold = crate::atlas::ATLAS_MAX_DIMENSION,
    ...
    pub(crate) fn load_from_rgba(
        ...
        match self.textures.entry(id) {
            ...
            Entry::Vacant(entry) => {
                ...
                if crate::atlas::fits_in_atlas(width, height) {
