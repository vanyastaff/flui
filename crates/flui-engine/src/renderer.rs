...
mod tests {
    ...
    fn on_top_layer_not_culled_by_opaque_background() {
        ...
        let mut tree = LayerTree::new(Layer::Opacity(OpacityLayer::new(1.0)));
        ...
        let _bg_layer_id = tree.push_child(
            root_id,
            Layer::Canvas(Box::new(CanvasLayer::from_canvas(bg_canvas))),
        );

        // child[1]: ImageFilterLayer(Blur σ=2.0) — bounds()=None (no occlusion check).
        let filter_id = tree.push_child(root_id, Layer::ImageFilter(ImageFilterLayer::blur(2.0)));
        ...
        let _fg_layer_id = tree.push_child(
            filter_id,
            Layer::Canvas(Box::new(CanvasLayer::from_canvas(fg_canvas))),
        );
    ...
    fn a_deep_shader_mask_chain_does_not_grow_the_call_stack() {
        ...
        use crate::offscreen::OffscreenRenderer;
        use crate::painter::WgpuPainter;
        use flui_layer::{Layer, LayerTree, ShaderMaskLayer};
        ...
        std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                let mut tree = LayerTree::new(mask());
                let mut parent = tree.root();
                for _ in 1..DEPTH {
                    let child = tree.push_child(parent, mask());
                    parent = child;
                }
    ...
    /// The offscreen a shader mask renders its subtree into must be
    /// DEVICE-sized (`bounds * dpr`), and the masked result composited at the
    /// device-space rect, with the DPR read from the live painter CTM. Under
    /// `scale(2)` a mask over logical (0,0,100,100) allocates 200x200 and
    /// composites at device (0,0,200,200); 100x100 means the CTM scale was
    /// dropped.
    #[test]
    fn shader_mask_offscreen_is_device_sized_under_dpr() {
        ...
        let surface = device.create_texture(&wgpu::TextureDescriptor {
            label: Some("ShaderMask HiDPI Test Surface"),
            size: wgpu::Extent3d {
                width: 800,
                height: 800,
                depth_or_array_layers: 1,
    ...
    fn shader_mask_nested_under_offset_ancestor_lands_at_correct_position() {
        ...
        let mask_id = tree.push_child(
            offset_id,
            Layer::ShaderMask(ShaderMaskLayer::new(shader, BlendMode::SrcOver, bounds)),
        );
