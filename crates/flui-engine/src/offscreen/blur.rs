...
impl OffscreenRenderer {
    ...
    fn get_or_create_blur_pipelines(&mut self) -> &BlurPipelines {
        if self.blur_pipelines.is_none() {
            ...
            let downsample_module =
                self.device
                    .create_shader_module(wgpu::ShaderModuleDescriptor {
                        label: Some(ShaderType::DualKawaseDownsample.label()),
                        source: wgpu::ShaderSource::Wgsl(
                            ShaderType::DualKawaseDownsample.source_code().into(),
                        ),
                    });
            ...
            let upsample_module = self
                .device
                .create_shader_module(wgpu::ShaderModuleDescriptor {
                    label: Some(ShaderType::DualKawaseUpsample.label()),
                    source: wgpu::ShaderSource::Wgsl(
                        ShaderType::DualKawaseUpsample.source_code().into(),
                    ),
                });
