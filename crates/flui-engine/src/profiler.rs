...
// ---------------------------------------------------------------------------
//
// Gated on `testing` (real GPU required) AND `gpu-profiler` (the
...
//     --features testing,gpu-profiler -- --test-threads 1
...
mod gpu_live_tests {
    ...
    fn profiler_captures_real_non_empty_timings_on_capable_adapter() {
        let Some((adapter, device, queue, has_inside_encoders)) = acquire_profiler_test_device()
            ...
            crate::test_support::resolve_unavailable_gpu(
                "profiler test device acquisition returned None",
                crate::test_support::require_gpu(),
            );
            return;
        };
