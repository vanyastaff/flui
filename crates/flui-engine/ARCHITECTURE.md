...
There is **no `unsafe impl Sync`** anywhere in the crate. The remaining unsafe surface is the `buffer_pool.rs` 5-block disjoint-borrow primitive (pre-existing).
...
- **Net unsafe delta for this chain: 0.** The single existing `unsafe { instance.create_surface_unsafe(...) }` block in `Renderer::new` is required by wgpu's API contract and stays; the chain consolidated the two unsafe calls into one block with a documented SAFETY comment. Zero new unsafe blocks were added. *(Since superseded: ADR-0063 deleted that block and the `RawHandles` `unsafe impl Send` — net unsafe delta −2 blocks, −1 manual impl — because "required by wgpu's API contract" was never true; wgpu's safe `create_surface` over an owned target conveys the same display handle the multi-monitor fix needed.)*
