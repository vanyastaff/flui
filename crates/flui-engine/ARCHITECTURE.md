...
One `unsafe impl Send` in the crate: `unsafe impl Send for RawHandles` ([`renderer.rs`](src/wgpu/renderer.rs)) — a private newtype `Renderer` holds as its `raw_handles` field, carrying exactly `Option<RawWindowHandle>` and `Option<RawDisplayHandle>`, which hold `NonNull<c_void>` on some platforms and are therefore `!Send` by default. **`Renderer` itself carries no manual `Send` impl any more** (ADR-0045 decision 1): with `raw_handles: RawHandles` the only field that would otherwise be `!Send`, and `RawHandles: Send` proven at its own impl site, `Renderer: Send` follows by ordinary auto-derivation — a future field that is `!Send` and left outside `RawHandles` now fails `cargo check` instead of silently riding a blanket assertion. `docs/runtime-contract.toml`'s `forbidden_pattern` entries (a `Send` entry alongside the pre-existing `Sync` one) reject a hand-reintroduced blanket impl, workspace-wide.

The SAFETY comment on `RawHandles`' impl is **not** a single cross-platform argument — thread-affinity for these handles genuinely differs by platform, investigated per platform rather than assumed:
- **Win32**: safe. The HWND is an opaque value, not a pointer into thread-owned memory; `raw-window-handle` itself asserts `Win32WindowHandle: Send + Sync`, and off-thread DXGI surface creation against an HWND is a documented, common pattern.
- **AppKit: not safe today.** `raw-window-handle` documents `AppKitWindowHandle` as `!Send`/`!Sync` specifically because `NSView` requires the main thread, and `wgpu-hal`'s Metal backend enforces this in code — `Instance::create_surface` calls `raw_window_metal::Layer::from_ns_view`, which **panics** via `MainThreadMarker::new().expect(..)` off the main thread. Since ADR-0045 decision 1 designates `Renderer::recover()` (which calls this path) raster-affine, **recover() cannot run on a non-main raster thread on AppKit without hitting this panic** — a real, open gap against decision 1's stated plan, not resolved by this narrowing and not papered over in the SAFETY comment.
- **X11 (winit fallback)**: safe. Winit calls `XInitThreads()` unconditionally before opening its `Display*` and asserts `Send`/`Sync` on the connection wrapping it itself; the window handle (an XID) is already `Send + Sync` per `raw-window-handle`'s own assertions.
- **Wayland (winit fallback)**: not verified to the same standard as the above; no known blocker, lower confidence.
