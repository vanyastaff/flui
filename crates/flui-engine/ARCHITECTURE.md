...
One `unsafe impl Send` in the crate: `unsafe impl Send for RawHandles` ([`renderer.rs`](src/wgpu/renderer.rs)) — a private newtype `Renderer` holds as its `raw_handles` field, carrying exactly `Option<RawWindowHandle>` and `Option<RawDisplayHandle>`, which hold `NonNull<c_void>` on some platforms and are therefore `!Send` by default. The handles are extracted once at construction, read only under `&mut self` (`recover` / `build_windowed_gpu_stack`), never aliased across threads, and the owning window (flui-app's `App`) outlives the `Renderer` — the SAFETY invariant is documented at the impl site. **`Renderer` itself carries no manual `Send` impl any more** (ADR-0045 decision 1): with `raw_handles: RawHandles` the only field that would otherwise be `!Send`, and `RawHandles: Send` proven above, `Renderer: Send` follows by ordinary auto-derivation — a future field that is `!Send` and left outside `RawHandles` now fails `cargo check` instead of silently riding a blanket assertion. `port-check.sh`'s `ADR-0045/renderer-send-narrow` guard rejects a hand-reintroduced `unsafe impl Send for Renderer`. There is **no `unsafe impl Sync`** anywhere in the crate. Two further unsafe surfaces: the wgpu surface-creation block in `renderer.rs`, and the `buffer_pool.rs` 5-block disjoint-borrow primitive (pre-existing).
