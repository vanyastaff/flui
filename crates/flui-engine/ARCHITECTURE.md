...
| `DrawBatcher::superellipse_cache` ([`src/wgpu/batches/mod.rs`](src/wgpu/batches/mod.rs)) | `SuperellipsePathCache` (bounded, `Arc<Path>` values) | Single-mutator, currently unreached | Owned by the batcher behind the painter's exclusive borrow — no lock and no thread-local. Listed here because an earlier revision described it as a `thread_local! RefCell<HashMap<…>>` in `layer_render.rs`, which it stopped being when the cache moved to the painter. It has had **no caller at all** since issue #921 routed `ClipSuperellipseLayer` to the squircle SDF; issue #935 decides whether it goes. |
