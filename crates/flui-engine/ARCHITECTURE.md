...
**Where the offscreen is declined, and why neither exemption is keyed on shape.** `Backend::opens_offscreen` grants it only when the mode asks, a clip was actually installed, and no enclosing layer routes through a bounds-growing image filter.

- *No clip installed.* `WgpuPainter::clip_path` warns and returns without touching any state, so there is no edge for a group composite to composite against. This is read from `ClipOutcome`, the value that call returns — not from which `push_clip_*` was entered — so the day path clipping lands the offscreen follows it with nothing else to update. That paid off exactly once already: `ClipSuperellipseLayer` used to route its squircle through `push_clip_path` and therefore did not clip at all (issue #921). Giving it `LayerStateStack::push_clip_rsuperellipse` was the whole repair — the offscreen decision followed from `ClipOutcome` with nothing here to update, and `Clip::AntiAliasWithSaveLayer` began working for squircles in the same change. What still reaches `push_clip_path` is a genuine arbitrary path, and `RenderPhysicalShape`'s `Path` variant relies on a clip it does not get (issue #934).
...
**Alternatives rejected:** opening the layer around a path clip anyway, to be uniform with the reference, delivers isolation at a boundary that does not exist while the clip is still missing. Letting the clip open its offscreen inside a filter layer and relying on the discard's `tracing::warn!` trades a visible edge artefact for silent content loss on a live path — `RenderPhysicalModel` selects this mode by name and `flui-widgets`' `Flow` passes it.
