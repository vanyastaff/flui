...
- *No clip installed.* `WgpuPainter::clip_path` warns and returns without touching any state, so there is no edge for a group composite to composite against. This is read from `ClipOutcome`, the value that call returns — not from which `push_clip_*` was entered — so the day path clipping lands the offscreen follows it with nothing else to update. That matters concretely: issue #921 records that `ClipSuperellipseLayer` routes its squircle through `push_clip_path` and therefore does not clip at all, even though `GpuStateStack::clip_rsuperellipse` implements one. Routing it correctly is one line, and it must not also require remembering this decision.
