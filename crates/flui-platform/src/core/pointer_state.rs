...
use flui_types::{
    events::{PointerDeviceKind, PointerEventData},
    Event, Offset, PointerEvent,
};
...
///
/// // Multiple moves during frame
/// state.update_position(Offset::new(10.0, 20.0), PointerDeviceKind::Mouse);
/// state.update_position(Offset::new(15.0, 25.0), PointerDeviceKind::Mouse);
///
/// // Get coalesced event for frame processing
/// if let Some(event) = state.take_pending_move() {
///     // Only processes position (15.0, 25.0)
/// }
/// ```
...
pub struct PointerState {
    ...
    device_kind: Option<PointerDeviceKind>,
...
impl PointerState {
    ...
    pub fn device_kind(&self) -> Option<PointerDeviceKind> {
        self.device_kind
    }
    ...
    pub fn update_position(&mut self, position: Offset, device: PointerDeviceKind) {
    ...
    pub fn take_pending_move(&mut self) -> Option<Event> {
        self.pending_move
            .take()
            .map(|data| Event::Pointer(PointerEvent::Move(data)))
    }
    ...
    pub fn create_event_data(&self, device: PointerDeviceKind) -> PointerEventData {
...
mod tests {
    ...
    fn test_update_position() {
        ...
        assert_eq!(state.device_kind(), Some(PointerDeviceKind::Mouse));
        assert!(state.has_pending_move());
    }
    ...
    fn test_event_coalescing() {
        ...
        state.update_position(Offset::new(10.0, 20.0), PointerDeviceKind::Mouse);
        state.update_position(Offset::new(15.0, 25.0), PointerDeviceKind::Mouse);
        state.update_position(Offset::new(20.0, 30.0), PointerDeviceKind::Mouse);
