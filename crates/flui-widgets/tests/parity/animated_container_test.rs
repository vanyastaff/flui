...
#[derive(Clone, StatefulView)]
struct ZeroDurationSizeProbe {
    vsync: Vsync,
    width: Arc<Mutex<f32>>,
}

struct ZeroDurationSizeProbeState {
    vsync: Vsync,
    width: Arc<Mutex<f32>>,
}

impl StatefulView for ZeroDurationSizeProbe {
    type State = ZeroDurationSizeProbeState;

    fn create_state(&self) -> Self::State {
        ZeroDurationSizeProbeState {
            vsync: self.vsync.clone(),
            width: Arc::clone(&self.width),
        }
    }
...
impl ViewState<ZeroDurationSizeProbe> for ZeroDurationSizeProbeState {
    fn build(&self, _view: &ZeroDurationSizeProbe, _ctx: &dyn BuildContext) -> impl IntoView {
...
/// A `Duration::ZERO` implicit animation retargeted in a rebuild settles
/// SYNCHRONOUSLY inside `did_update_view` (issue #1171:
/// `ImplicitController::restart_from_zero` → `AnimationController::forward_from(Some(0.0))`
/// now returns an already-complete run instead of installing a ticker run),
/// so the new target is laid out on the SAME pump that observes the
/// widget's new configuration, and nothing is left running afterward for the
/// frame loop to keep the window open for.
///
/// Every `Duration::ZERO` elsewhere in this crate's tests is a pump `dt`,
/// never an animation `duration` — this is the only coverage of that path.
///
/// Red-check: revert the zero-duration settle in `AnimationController` (or
/// gate `forward_from`'s settle on distance alone) and the width read right
/// after `laid.pump()` still reads the OLD target, one frame early of where
/// this test looks.
#[test]
fn zero_duration_retarget_lays_out_the_new_target_on_the_same_pump() {
    let vsync = Vsync::new();
    let width = Arc::new(Mutex::new(100.0));
    let probe = ZeroDurationSizeProbe {
        vsync: vsync.clone(),
        width: Arc::clone(&width),
    };
    let mut laid = lay_out_animated(probe, loose(400.0), vsync.clone());

    assert_eq!(
        laid.size(laid.current_root()).width.get(),
        100.0,
        "sanity: the initial mount settles at the first target with no motion"
    );
    assert!(
        !vsync.has_running(),
    ...
    *width.lock() = 250.0;
    ...
    assert_eq!(
        ...
        "a Duration::ZERO retarget must lay out the new target on the SAME \
         pump that rebuilds with it, not one frame later"
    );
    assert!(
        !vsync.has_running(),
