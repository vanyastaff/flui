...
pub struct PointerContacts {
    /// Next contact's pointer id.
    next: Cell<u64>,
    /// Pointer id of the in-flight contact, shared by its Move/Up/Cancel.
    /// Zero means "no contact has begun yet" — [`PointerId`] itself starts
    /// at one, so the sentinel cannot collide with a real contact.
    current: Cell<u64>,
}

impl Default for PointerContacts {
    fn default() -> Self {
        Self::new()
    }
}
...
impl PointerContacts {
    /// A tracker with no contact begun yet.
    pub fn new() -> Self {
        Self {
            next: Cell::new(1),
            current: Cell::new(0),
    ...
    /// # Panics
    ///
    /// Panics when no contact has begun — a synthetic Move/Up/Cancel without
...
impl LaidOut {
    ...
    /// render root, so render-tree inspection alone cannot observe that
    ...
    /// The unique render node whose short type name equals
    /// `render_type_name`, or `None` when nothing matches.
    ///
    /// The value-returning sibling of
    /// [`find_by_render_type`](Self::find_by_render_type): callers that
    /// assert "not (yet) mounted" need the no-match case to be a value, not
    /// a panic.
    ///
    /// # Panics
    ///
    ...
    /// [`find_all_by_render_type`](Self::find_all_by_render_type) when
    /// duplicates are expected).
    pub fn try_find_by_render_type(&self, render_type_name: &str) -> Option<RenderId> {
        let matches = self.find_all_by_render_type(render_type_name);
        match matches.as_slice() {
            [id] => Some(*id),
            [] => None,
            _ => panic!(
                "try_find_by_render_type: {} render nodes named {render_type_name:?}; \
                 use find_all_by_render_type when duplicates are expected",
                matches.len()
            ),
        }
