...
fn bench_color_blending(c: &mut Criterion) {
    ...
    group.bench_function("color_lerp", |b| {
        ...
        let c2 = Color::BLUE;
        b.iter(|| {
            black_box(Color::lerp(c1, c2, 0.5))
        });
    });
    ...
    group.bench_function("color_blend_batch", |b| {
        ...
        let bg = Color::WHITE;
        b.iter(|| {
            black_box(Color::blend_over_batch(&colors, bg))
        });
    });
...
fn bench_matrix_operations(c: &mut Criterion) {
    ...
    group.bench_function("matrix_transform_point", |b| {
        ...
        b.iter(|| {
            black_box(m.transform_point(5.0, 5.0))
        });
    });
    ...
    group.bench_function("matrix_transform_points_batch", |b| {
        ...
        b.iter(|| {
            black_box(m.transform_points(&points))
        });
    });
...
fn bench_batch_operations(c: &mut Criterion) {
    ...
    for size in [10, 50, 100, 500].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("transform_single", size),
            size,
            |b, _| {
                b.iter(|| {
        ...
        group.bench_with_input(
            BenchmarkId::new("transform_batch", size),
            size,
            |b, _| {
                b.iter(|| {
                    black_box(m.transform_points(&points))
