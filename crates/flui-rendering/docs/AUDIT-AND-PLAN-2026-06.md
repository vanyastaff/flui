...
- `objects/sliver_list_lazy.rs:546-547` (+4 сливера) — `has_visual_overflow = … || scroll_offset>0`
  → клип-слой на **каждом** проскролленном списке, даже если контент влезает. Убрать дизъюнкт.
...
1.5 `has_visual_overflow`: убрать `|| scroll_offset>0.0` в 5 сливерах (стоп over-clip).
