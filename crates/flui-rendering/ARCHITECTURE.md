...
**Also new: a patch that panics poisons the frame instead of being refused.**
...
(`RenderError::Poisoned { phase: "layer-update", .. }`), the frame is
discarded whole, and the queued update survives on the node for the retry.
...
- `PipelineOwner::<PaintPhase>::paint_subtree_impl` ([`src/pipeline/owner/paint.rs`](src/pipeline/owner/paint.rs)) — not `render_object.paint(context, offset)`, which is no longer a call site here — wraps `render_node.paint_raw(&mut recorder, ...)` (the fragment recorder) together with the node's own effect-layer build, `own_effect_layers(render_node.paint_effects(), origin)`, in ONE `catch_unwind`. Both run only after the walk's three return gates (`skip_paint`, `needs_layout`, the sliver visibility cull): a gated-out node never builds a `PaintEffects` descriptor it will not use — before this move a `needs_layout` node ran `paint_effects` anyway, against a stale `Size::ZERO`. A panic in either half — the node's own `paint_effects`, or the walk's resolution of a `PaintClip::PathTarget` inside `own_effect_layers`'s clip arm — surfaces as `Poisoned { phase: "paint", .. }`, one poison point per node rather than two.
- `PipelineOwner::<PaintPhase>::layer_patches_for` ([`src/pipeline/owner/paint.rs`](src/pipeline/owner/paint.rs)), the composited-layer-update patch arm, wraps the same `own_effect_layers(node.paint_effects(), slots.origin)` rebuild — run once per target being patched into a retained boundary's capture — in its own `catch_unwind`. A panic surfaces as `Poisoned { phase: "layer-update", .. }` and the function returns `Err` instead of `Ok(None)`: the whole frame is discarded and the queued composited-layer-update request survives on the node for the retry. Falling back to a repaint instead (the way a structural shape mismatch does, via `Ok(None)`) was rejected — the repaint would call the same panicking `paint_effects` on the same node and panic a second time.
...
`Poisoned`'s `phase` values are `"layout"`, `"paint"`, and `"layer-update"` (enumerate with `rg -n "poisoned\(" crates/flui-rendering/src`); nothing today produces a fourth. This whole per-node partial-failure-recovery mechanism — every wrapper in this section, the two new ones included — has no Flutter counterpart: a throwing `paint`/`updateCompositedLayer` propagates through `FlutterError.reportError`, and the `_debugDoingThisPaint` assertion machinery aborts the frame outright (`object.dart`, 3.44.0) rather than isolating the failing node.
