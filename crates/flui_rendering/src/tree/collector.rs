    ...
    pub fn single(&self) -> ElementId {
        ...
        #[cfg(not(debug_assertions))]
        {
            ...
            if self.children.len() > 1 {
                panic!(
                    "Single arity accessor: expected exactly 1 child, found {}",
                    self.children.len()
                );
            }
        }
...
impl<const INLINE_SIZE: usize> CollectedChildren<Leaf, INLINE_SIZE> {
    ...
    pub fn validate_empty(&self) {
        ...
        #[cfg(not(debug_assertions))]
        {
            if !self.is_empty() {
                panic!(
                    "Leaf arity accessor: expected no children, found {}",
                    self.len()
                );
            }
        }
...
mod tests {
    ...
    fn test_into_optional() {
        ...
        let empty_collector: RenderChildrenCollector =
            RenderChildrenCollector::new(&tree, empty_parent);
    ...
    fn test_into_leaf() {
        ...
        let collector: RenderChildrenCollector = RenderChildrenCollector::new(&tree, leaf);
    ...
    fn test_try_into_arity() {
        ...
        let collector2: RenderChildrenCollector = RenderChildrenCollector::new(&tree, parent);
