...
mod tests {
    ...
    fn test_sliver_geometry() -> SliverGeometry {
        SliverGeometry::new(100.0, 100.0, 0.0)
    }

    #[test]
    fn test_constraints_protocol() {
        let box_c = Constraints::Box(BoxConstraints::tight(Size::new(100.0, 100.0)));
        assert_eq!(box_c.protocol(), ProtocolId::Box);
        assert!(box_c.is_box());
        assert!(!box_c.is_sliver());

        let sliver_c = Constraints::Sliver(test_sliver_constraints());
    ...
    fn test_geometry_protocol() {
        ...
        let sliver_g = Geometry::Sliver(test_sliver_geometry());
    ...
    fn test_from_conversions() {
        let box_c: Constraints = BoxConstraints::tight(Size::new(100.0, 100.0)).into();
        assert!(box_c.is_box());

        let sliver_c: Constraints = test_sliver_constraints().into();
        assert!(sliver_c.is_sliver());

        let box_g: Geometry = Size::new(100.0, 100.0).into();
        assert!(box_g.is_box());

        let sliver_g: Geometry = test_sliver_geometry().into();
        assert!(sliver_g.is_sliver());
    }
}
