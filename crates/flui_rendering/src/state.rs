...
///
/// This trait reduces boilerplate when working with type-erased render states
/// returned from `RenderTreeAccess::render_state()`. Instead of manually
/// downcasting to `BoxRenderState` or `SliverRenderState`, use these helper
/// methods.
///
/// # Example
///
/// Before (verbose):
/// ```rust,ignore
/// if let Some(state) = storage.render_state(id) {
...
/// After (concise):
/// ```rust,ignore
/// use flui_rendering::core::RenderStateExt;
///
/// if let Some(state) = storage.render_state(id) {
///     return state.offset();  // Tries both protocols automatically
/// }
/// ```
///
/// # Design Rationale
///
/// The render state is returned as `&dyn Any` from `flui-tree` to maintain
/// decoupling between crates. This extension trait provides ergonomic access
/// without changing the underlying architecture.
pub trait RenderStateExt {
    /// Attempts to downcast to `BoxRenderState`.
    ///
    /// Returns `Some(&BoxRenderState)` if the underlying type is `BoxRenderState`,
    /// `None` otherwise.
    fn as_box_state(&self) -> Option<&BoxRenderState>;

    /// Attempts to downcast to `SliverRenderState`.
    ///
    /// Returns `Some(&SliverRenderState)` if the underlying type is `SliverRenderState`,
    /// `None` otherwise.
    fn as_sliver_state(&self) -> Option<&SliverRenderState>;

    /// Gets the offset from either Box or Sliver protocol.
    ///
    /// Tries `BoxRenderState` first (most common), then `SliverRenderState`.
    fn offset(&self) -> Option<Offset>;

    /// Checks if layout is needed (Box protocol only for now).
    ///
    /// Returns `false` if the state is not a `BoxRenderState`.
    fn needs_layout(&self) -> bool;

    /// Checks if paint is needed (Box protocol only for now).
    ///
    /// Returns `false` if the state is not a `BoxRenderState`.
    fn needs_paint(&self) -> bool;

    /// Gets the render flags (Box protocol only).
    ///
    /// Returns `None` if the state is not a `BoxRenderState`.
    fn render_flags(&self) -> Option<&AtomicRenderFlags>;

    /// Clears the needs_layout flag (Box protocol only).
    ///
    /// No-op if the state is not a `BoxRenderState`.
    fn clear_needs_layout(&self);

    /// Clears the needs_paint flag (Box protocol only).
    ///
    /// No-op if the state is not a `BoxRenderState`.
...
// Also implement for the non-Send+Sync version for flexibility
