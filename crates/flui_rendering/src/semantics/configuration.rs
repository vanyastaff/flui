...
// Note: Offset and Rect not currently used but may be needed for future features

use super::{
    AttributedString, CustomSemanticsAction, SemanticsAction, SemanticsActionHandler,
    SemanticsFlag, SemanticsFlags, SemanticsHintOverrides, SemanticsProperties, SemanticsSortKey,
...
pub struct SemanticsConfiguration {
    ...
    actions: HashMap<SemanticsAction, SemanticsActionHandler>,
    ...
    custom_actions: Vec<CustomSemanticsAction>,

    /// Tags for this node.
    tags: Vec<SemanticsTag>,
...
mod tests {
    ...
    fn test_button_configuration() {
        ...
        assert_eq!(config.label().map(|l| l.string.as_str()), Some("Submit"));
        assert!(config.has_content());
    }
    ...
    fn test_slider_configuration() {
        ...
        assert_eq!(config.value().map(|v| v.string.as_str()), Some("50%"));
        assert_eq!(
            config.increased_value().map(|v| v.string.as_str()),
            Some("55%")
        );
        assert_eq!(
            config.decreased_value().map(|v| v.string.as_str()),
            Some("45%")
        );
    }
    ...
    fn test_configuration_absorb() {
        ...
        assert_eq!(
            parent.label().map(|l| l.string.as_str()),
            Some("Child label")
        );
    ...
    fn test_from_properties() {
        ...
        assert!(config.is_button());
        assert_eq!(config.is_enabled(), Some(true));
        assert_eq!(config.label().map(|l| l.string.as_str()), Some("Test"));
    }
}
