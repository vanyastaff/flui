...
impl Hash for SliverPhysicalParentData {
    fn hash<H: Hasher>(&self, state: &mut H) {
        self.paint_offset.x.to_bits().hash(state);
        self.paint_offset.y.to_bits().hash(state);
    }
}
...
mod tests {
    ...
    fn test_sliver_grid_parent_data() {
        let data = SliverGridParentData::new(3, 50.0)
            .with_layout_offset(100.0);
    ...
    fn test_sliver_physical_parent_data() {
        ...
        assert_eq!(data.paint_offset.x, 10.0);
    }
}
