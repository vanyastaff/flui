...
use super::box_parent_data::BoxParentData;
...
impl Hash for StackParentData {
    fn hash<H: Hasher>(&self, state: &mut H) {
        ...
        let hash_opt_f32 = |value: Option<f32>, state: &mut H| {
            match value {
                Some(v) => {
                    true.hash(state);
...
mod tests {
    ...
    fn test_container_box_parent_data() {
        let data = ContainerBoxParentData::zero()
    ...
    fn test_multi_child_layout_parent_data() {
        let data = MultiChildLayoutParentData::zero()
            .with_id("child1".to_string());
