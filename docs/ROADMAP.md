...
- **Navigator API gaps** (surfaced by the parity port, 2026-07-15): no `pop_until`, no `RouteSettings` arguments field, no `onGenerateRoute`/named-route generation, and the `Navigator` is imperative-only (no declarative `Pages` API) — each is a real Flutter surface the catalog still owes.
