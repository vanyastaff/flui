...
| Business.1 — widget catalog | ◐ Every named catalog widget implemented and integration-tested; named `Hero` gaps are closed (cross-navigator flights + user-gesture flights, `ROADMAP-TRACKER.md` B1.4). **Fidelity** (ported parity corpus) is the only Business.1 residue remaining. |
...
- ~~**Named `Hero` gaps**~~ — ✅ CLOSED (2026-07-15, `ROADMAP-TRACKER.md` B1.4). Cross-navigator flights and user-gesture flights (`Hero.transitionOnUserGestures`, driven by a new private `BackGestureController` edge-drag substrate) are both delivered; the prior "blocked on FLUI having no back-swipe" note (ADR-0020 §7e) no longer applies. Remaining Cupertino-specific swipe-back visuals are Catalog.1 territory, not a Business.1 gap.
