...
| Business.1 — widget catalog | ◐ Every named catalog widget implemented and integration-tested; the cross-navigator-flights and user-gesture-flights `Hero` gaps are closed (`ROADMAP-TRACKER.md` B1.4) — one further named `Hero` residue remains (heroes do not fly on `push_replacement`, below). **Fidelity** (ported parity corpus) is the primary Business.1 residue remaining. |
...
- **Named `Hero` gaps** — cross-navigator flights and user-gesture flights are ✅ CLOSED (2026-07-15, `ROADMAP-TRACKER.md` B1.4): `Hero.transitionOnUserGestures`, driven by a new private `BackGestureController` edge-drag substrate, is delivered; the prior "blocked on FLUI having no back-swipe" note (ADR-0020 §7e) no longer applies. Remaining Cupertino-specific swipe-back visuals are Catalog.1 territory, not a Business.1 gap. **Still owed:** heroes do not fly on `push_replacement` — Flutter's `pushReplacement` still fires `didChangeTop` (the oracle: `heroes_test.dart`'s `'Heroes fly on pushReplacement'`), but `HeroController` overrides only `did_change_top` and `NavigatorHandle::push_replacement` does not currently route through it, so no flight starts on a `push_replacement` today. A real feature gap (wiring `push_replacement` through the hero flight path), not a harness limitation — surfaced and named by `crates/flui-widgets/tests/parity/heroes_test.rs`'s "Not ported" list, not silently dropped.
...
- **`flui-assets` ↔ `Image` integration verification** — ✅ VERIFIED, gap confirmed and named (2026-07-15). `flui-assets` works standalone: `ImageAsset::file`/`AssetRegistry::load` decode a real committed PNG fixture to its true dimensions and serve it from cache on a second lookup (`crates/flui-assets/tests/image_asset_integration.rs`), and the `network` feature's `NetworkLoader::load_url` round-trips bytes against a hermetic local HTTP server (`crates/flui-assets/src/loaders/network.rs`). **But `flui-assets` is NOT wired into the `Image` widget** — `flui-widgets` has no dependency on `flui-assets` at all; `Image` resolves through its own, independent `ImageProvider` trait (`src/image/provider.rs`) backed directly by the `image` crate (verified against a real committed PNG fixture in `crates/flui-widgets/tests/image.rs`). `flui-engine`'s `TextureCache::load_from_asset` (behind its `assets` feature) is a same-signature rename of `load_from_rgba`, not a call into `flui_assets`; its doc-comment example wiring `AssetRegistry` through it is `rust,ignore`, unexercised. `flui-widgets`' `network-images` feature is a typed stub (`NetworkImage::resolve` unconditionally returns `AsyncNotWired`) that never issues a request, so there is no network path through the widget to test hermetically. Font loading bypasses `flui-assets` entirely: `flui-engine`'s `FontLoader` (`src/wgpu/font_loader.rs`) loads bytes/files directly into `glyphon`'s `FontSystem`; `flui_assets::FontAsset` has no consumer outside its own crate. Wiring `flui-assets` into `Image`/font loading is an architectural change (bridging `flui-assets`' async `Asset`/`AssetRegistry` API into the Image widget's sync `ImageProvider` resolve path — the same async-rebuild gap `ImageProvider`'s module doc already names as deferred), not a localized adapter; not attempted here.
