...
| App.1 — application integration | ◐ `run_app`, both bindings, and a wake-driven, vsync-paced frame loop exist (`Fifo`-blocking present + `ControlFlow::Wait`, measured on Linux/Wayland — [ADR-0029](adr/ADR-0029-frame-pacing-swapchain-block-with-fallback-throttle.md)); IME, the facade crate, and native-backend pacing evidence (Cross.P) remain. |
