...
| U19 (частично) | Workspace-линты: `unexpected_cfgs`, `unsafe_op_in_unsafe_fn`, `unused_must_use` = `deny` (замерено 0 сайтов, значит это барьер против регрессии, а не миграция). `rust_2018_idioms` опущен до priority -1 | `clippy --all-targets -D warnings` чисто |
