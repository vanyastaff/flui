...
| U2 | Гарды опираются на **уже существовавшие** `can_activate` (только `Inactive`) и `can_deactivate` (только `Active`), проверяемые `debug_assert` в `ElementCore::{mount,activate,deactivate}` и в рукописных элементах `RootRenderElement`/`ErrorElement`. Введённый сначала `can_transition_to` был **слабее** этих двух и удалён: его обоснование («обходы дерева легитимно повторно активируют уже-Active элемент») оказалось выдумкой — со строгими предикатами весь суite зелёный | Два `#[should_panic]`-теста (debug-only: гард — `debug_assert`) доводят элемент до `Defunct`; третий проходит обе таблицы целиком. **Не покрыто:** переходы в `Defunct` не гардятся (они легальны из любого состояния), а `mount` у рукописных элементов гарда не получил |
...
| U19 | **Частично, и `undocumented_unsafe_blocks` откатан.** Приняты `unexpected_cfgs`, `unsafe_op_in_unsafe_fn`, `unused_must_use` = `deny` (замерено 0 сайтов — барьер против регрессии). `undocumented_unsafe_blocks` был включён и **снят обратно** по двум причинам, зафиксированным в `Cargo.toml`: (1) он видит только то, что компилирует Linux-джоб, а в `cfg(windows)`/`cfg(macos)` бэкендах `flui-platform` остаются ~91 сайт, до которых не доходит ни один гейт — то есть зелёный прогон лицензировал бы ложное утверждение; (2) включить линт первым, а комментарии написать вторыми — значит производить прозу, а не ревью, и именно так несколько комментариев в `flui-hot-reload` стали утверждать инварианты, которых код не обеспечивает. Комментарии, оказавшиеся верными, оставлены; семь ложных переписаны | `clippy --all-targets -D warnings` чисто; ревью ветки воспроизвело 91 сайт на `x86_64-pc-windows-msvc` |
...
| Кросс-таргеты | **Оба нативных бэкенда были сломаны и это не мог увидеть ни один гейт.** Windows не собирался вовсе (`pub mod win32` без документации под `#![deny(missing_docs)]`); девять типов в Windows и macOS без `Debug`. Добавлен джоб `cross-check` + `just cross-typecheck` | `cargo check` не линкует, поэтому оба бэкенда проверяются с Linux; оба чисты под `-D warnings` |
...
**Про `undocumented_unsafe_blocks`.** Этот абзац изначально говорил «НЕ принят», потому что SAFETY-комментарий обязан доказывать инвариант, а массовая простановка сфабриковала бы непроверенные утверждения. Затем линт всё же был включён — и предсказание сбылось: ревью нашло три комментария с ложными посылками (`dlerror` объявлен потокобезопасным, хотя POSIX §2.9.1 его исключает; уникальность владения `DynLib` выведена из отсутствия `Clone`, тогда как `dlopen` возвращает тот же handle с refcount; `drop_in_place` описан как «только флип `AtomicBool`»). Линт снят, ложные комментарии переписаны. Вывод для будущих заходов: сначала ревизия soundness, потом линт — не наоборот.
...
**Остаётся открытым:** U3, U6, U7, U8, U9, U15–U18, U20 и остаток U19.

---

## Статус: ремонтный проход 2026-07-28

Каждый пункт из «Остаётся открытым» проверен против HEAD перед починкой
(часть цитат аудита устарела за ~40 коммитов — расхождения зафиксированы
ниже), затем закрыт кодом или дизайном:

| Пункт | Действие | PR |
|---|---|---|
| U15 | Проверка показала, что описание было завышено: `flui-app/runner.rs` и `flui-binding/lib.rs` имели `SeqCst` только в test/doctest-коде, а `AtomicRenderFlags` уже жил на документированной схеме Acquire/Release/AcqRel (и на срезе аудита тоже). Реальный остаток — 19 production-сайтов в трёх файлах (flui-semantics binding — 8, dismissible — 7, android `running` — 4), все «голые счётчики/флаги без публикации данных» → `Relaxed` с письменным обоснованием у каждой группы; `NEXT_IDENTITY` уже был `Relaxed` — дописано обоснование; в `flags.rs` зафиксировано, что кросс-поточного читателя сегодня нет и схема консервативна намеренно | #482 |
| Wasm-часы | `SystemClock` и весь pointer-путь flui-interaction переведены на `web-time` (std-реэкспорт на native, `performance.now()` на wasm32); свип компиляторно-проверяемый — на wasm32 это разные типы. Устаревшее в аудите: `draggable.rs`/`back_gesture.rs` к HEAD были test-only; wasm-bindgen в workspace уже есть (web_demo/painting_demo — cdylib) | #483 |
| Арена/miri | Гард `baseline_cb_owned` уже был починен (e2d81942, в день аудита); добавлены два real-NodePtr теста, которые гоняют `layout_dirty_root` под miri (прямой проход + циклическое ребро; снятие гарда воспроизводимо валит miri: «would remove [Unique] which is strongly protected»); комментарии о scope miri в justfile/AGENTS.md обновлены | #484 |
| Остаток U19 | cross-typecheck переведён с `cargo check` на `cargo clippy -- -D warnings`; вскрылось ~90 deny-нарушений в win32/appkit бэкендах — все починены, ни одно не подавлено. Это записанное в Cargo.toml предусловие для возврата к `undocumented_unsafe_blocks` | #485 |
| U8 | `flui-reactivity` удалён (вариант A): 0 потребителей, не собирался даже standalone (наследовал необъявленные workspace-зависимости), C1 запирает каталог на setState/Inherited, а его процесс-глобальный `SIGNAL_RUNTIME` противоречит UiRealm-модели (M16). Живые доки обновлены; история в git | #486 |
| Hot-reload граница | Все три дефекта из «Заведено отдельно»: (1) реестр `WORKER_BUILDS` прочищается в `Drop` воркера ДО unmap — `Some` теперь означает живой образ; (2) Scene-коробки деаллоцируются плагином (`ptr::read` + новые `flui_*_free`), а не host-glue; (3) введён ABI-токен-хендшейк (`flui_*_abi_token`: rustc -vV + версия крейта + size/align Scene/LayerTree) — загрузчики отказывают при несовпадении. U16 закрыт этим же хендшейком; `WorkerPollOutcome::Degraded` наконец производится при смене fingerprint | #487 |
| U17 | Поправка к постановке: backend-фичи **аддитивны по дизайну** (pass-through к wgpu, выбор в рантайме по target_os; ноль cfg-сайтов) — «взаимоисключающие» было ложной посылкой уже на срезе аудита. Сделано то, что реально открыто: документированная матрица комбинаций в flui-engine/ARCHITECTURE.md + bounded pairwise powerset (depth 2, ~22 конфигурации) в feature-matrix | #488 |
| U20 | `scripts/bench-collect.sh` (перечисляет настоящие `[[bench]]`-таргеты — голый `cargo bench -- --save-baseline` падает на libtest-харнесах, воспроизведено), `just bench-save`/`bench-compare` для локального A/B, advisory-джоб в weekly.yml с 90-дневным артефактом. Намеренно не merge-gate | #488 |
| Остаток U11 | wasm-check реально линкует оба cdylib-демо и сверяет import-модули с закоммиченным allowlist (`scripts/wasm-import-allowlist.txt`) — на wasm32 undefined symbol не валит линк, а становится импортом, поэтому список и есть проверяемое утверждение | #488 |
| U3 (доки) | Ложная реклама убрана: network-monitor/memory-profiler/remote-debug и «State preservation» удалены из доков flui-devtools; фантомные feature-блоки фасада удалены; крейт честно описан как «не инспектор — нужен observation seam» | #489 |
| U9 | Сначала замер (llvm-lines), потом фикс: V-независимая половина `ElementCore` вынесена в не-генеричный `CoreState`, холодные lifecycle-методы компилируются один раз. flui-widgets: 669 653 → 561 583 LLVM-строк (−16.1%); release-бинарь (screenshot, thin-LTO, честный A/B) −234 KiB (−1.1%). `dispatch_view_update` намеренно не тронут — это K2 (клоны), не outlining | #490 |
| U12 | Подтверждено починенным на HEAD (единственный канонический `TextRange` в flui-types с Copy/Eq/Hash); строка в сводной таблице ниже исправлена. Уточнение: гейт дублей имён — триггер #10, не #14 | — |
| U6+U18, U7, U3 (seam) | «Требует проектирования» → спроектировано: ADR-0038 (data-transfer: 7-стадийная модель, DataTransferId на генерационных ID, доставка через TaskToken, фасады Clipboard/Drag), ADR-0039 (event-loop affinity: !Send-capability на базе ControlSender-лейна — foreground_executor, на который рассчитывал аудит, к HEAD удалён), ADR-0040 (tree-observation seam для devtools). Реализация — слайсами по ADR | см. ADR |

Флейк `dismissible` из заметки выше при прогонах не воспроизвёлся (тест зелёный, в т.ч. под полным `just ci`); попутно найден и починен ДРУГОЙ флейк той же природы «нагруженного прогона» — tracing-capture в flui-view (interest-cache гонка; общий фикстур с постоянным глобальным подписчиком, #484).

**Wasm: гейт зелёный ≠ wasm работает.** `flui-layer` починен (`web-time`), но `SystemClock` в `flui-foundation` — это `std::time::Instant::now()`, и он же **дефолтный** clock у `GestureArena` (`flui-interaction`), плюс те же вызовы в `timer.rs`, `sampling_clock.rs`, `draggable.rs`, `back_gesture.rs`. На wasm приложение паникует на первом же pointer-событии, а `run_web` — живой путь. `cargo check` этого не видит принципиально. Pre-existing, вне этой ветки; зафиксировано, чтобы «wasm-check проходит» не превратилось в «wasm работает».

**Заведено отдельно (pre-existing, не вносилось этой веткой).** Ревью вскрыло четыре дефекта границы плагина и арены, которые SAFETY-комментарием не лечатся: `subtree_arena` `baseline_cb_owned` — единственный из шести child-deref сайтов без гарда `is_in_flight`; `WORKER_BUILDS` никогда не очищается, поэтому после `unload()` в карте остаётся адрес в размапленный код; `flui_scene_drop` резолвится и не вызывается, а `Box<Scene>` с `repr(Rust)` освобождается host-овой glue через границу cdylib; miri-гейт на `subtree_arena` покрывает 5 unit-тестов, ни один из которых не разыменовывает реальный `NodePtr`.
...
| U2 | §18 | Гарды `Lifecycle` не применялись нигде в production; `Defunct → Active` был достижим | **Доказанный дефект — ИСПРАВЛЕНО** (`ElementCore` + `RootRenderElement`/`ErrorElement`; `mount` рукописных элементов — нет) |
| U3 | §26 | `flui-devtools` не зависит ни от одного flui-крейта → не может наблюдать дерево; документация рекламирует 3 несуществующие подсистемы | **Доказанный дефект — доки ИСПРАВЛЕНЫ (#489); observation seam СПРОЕКТИРОВАН (ADR-0040)** |
...
| U6 | §22 | Системный drag-and-drop отсутствует полностью; clipboard синхронный/text-only/MIME строкой | **Доказанный дефект — СПРОЕКТИРОВАНО (ADR-0038)**, реализация слайсами |
| U7 | §23 | Ни одной проверки main-thread; все ~30 методов `Platform` вызываемы из любого потока | **Доказанный дефект — СПРОЕКТИРОВАНО (ADR-0039)**; к HEAD winit `open_window` уже owner-thread-маршрутизирован, `foreground_executor` удалён |
| U8 | §20/§29 | `flui-reactivity` вне workspace: не компилируется, не линтуется, не тестируется; edition 2021 | **Доказанный дефект — ЗАКРЫТ удалением крейта (#486)** |
| U9 | §21 | 29 573 мономорфизации в `flui-widgets`; холодные lifecycle-методы — 13.7% кода крейта | **Доказанный дефект — ИСПРАВЛЕНО (#490)**: −16.1% LLVM-строк крейта, −234 KiB release-бинаря |
...
| U11 | §19 | wasm проверяется только `cargo check` → линковка, undefined symbols и список импортов не проверяются | **Доказанный дефект — ИСПРАВЛЕНО (#488)**: реальная линковка двух cdylib + allowlist импорт-модулей |
| U12 | §15 | Два разных публичных `TextRange` в `flui-types` и `flui-rendering` | **Доказанный дефект — ИСПРАВЛЕНО** (консолидация в flui-types; см. «Статус» выше) |
...
| U15 | §17 | Ordering в production-местах не обоснован комментариями; `RenderFlags` вероятно требует `Acquire`/`Release` | **Риск — ЗАКРЫТ (#482)**; поправка: `RenderFlags` уже был на Acquire/Release, а два из пяти «production»-файлов имели SeqCst только в тестах |
| U16 | §28 | dlopen hot-reload: изменение layout типа между сборками не проверяется на совместимость | **Риск — ЗАКРЫТ (#487)**: ABI-токен-хендшейк при загрузке + Degraded при смене fingerprint |
| U17 | §29 | Взаимоисключающие backend-features `flui-engine` не проверяются на комбинации (`--each-feature` ≠ powerset) | **Риск — ЗАКРЫТ (#488)**; поправка: фичи аддитивны по дизайну, «взаимоисключающие» — ложная посылка; сделаны матрица + pairwise powerset |
| U18 | §22 | Синхронный `read_text()` на Wayland/X11 — блокировка UI-потока | **Риск** |
| U19 | §20 | `unsafe_code = "warn"`, не `deny`; нет `undocumented_unsafe_blocks` | **Риск — остаток ЗАКРЫТ (#485)**: cross-typecheck на clippy, ~90 нарушений починено; `undocumented_unsafe_blocks` можно возвращать после ревизии SAFETY-прозы |
| U20 | §30 | 21 бенчмарк компилируется, но числа не собираются и не сравниваются → регрессии не видны | **Риск — ЗАКРЫТ (#488)**: bench-collect + локальный A/B + weekly-артефакт (advisory) |
...
**Ремонтный проход 2026-07-28 закрыл весь список** (см. раздел «Статус:
ремонтный проход 2026-07-28» выше): U12 подтверждён починенным, U8 закрыт
удалением, остаток U19 — #485, U11 — #488, U3-доки — #489, U9 — #490,
U15/U16/U17/U20 — #482/#487/#488, wasm-часы и дефекты hot-reload-границы —
#483/#487. Проектные пункты (U6+U18, U7, U3-seam) спроектированы как
ADR-0038/0039/0040; их реализация идёт слайсами по этим ADR и больше не
отслеживается этим документом.
