...
| U19 | §20 | `unsafe_code = "warn"`, не `deny`; нет `undocumented_unsafe_blocks` | **Риск — ЧАСТИЧНО** (3 линта приняты; `undocumented_unsafe_blocks` включался и откатан — см. «Статус») |
...
**Ближайшее (дни):** U8 (решить судьбу `flui-reactivity`); остаток U19 — сначала заставить кросс-таргетный джоб гонять `clippy`, а не `cargo check`, иначе линт снова будет лицензировать ложное утверждение.
