...
**Остаётся открытым:** U2, U3, U5, U6, U7, U8, U9, U12, U15–U20.
...
| U2 | §18 | Гарды `Lifecycle` (`can_activate`/`can_deactivate`) не применяются нигде в production; `Defunct → Active` достижим | **Доказанный дефект** |
...
| U5 | §27 | `show_performance_overlay` никогда не читается; `PerformanceOverlayLayer` не конструируется; features `*-overlay` пустые | **Доказанный дефект** |
...
| U19 | §20 | `unsafe_code = "warn"`, не `deny`; нет `undocumented_unsafe_blocks`, `unexpected_cfgs`, `unsafe_op_in_unsafe_fn` | **Риск** |
...
**Сделано:** U1, U4, U10, U13, U14 (см. раздел «Статус» выше).
**Немедленно (часы):** U12.
**Ближайшее (дни):** U2 (`debug_assert` на переходах), U5 (~50 строк соединительного кода), U8 (решить: вернуть в workspace или удалить).
**Средний срок (недели):** U11 (настоящая линковка + список импортов в CI), U19, U3 (инверсия зависимости devtools).
