...
| Windows (Win32) | **experimental** | `flui-platform`'s suite on windows-latest (the `platform-windows` CI job, default and all-features); a live window driven through UI Automation, the API Narrator uses — the generated counter's texts read by name, the button pressed through `IUIAutomationInvokePattern`, the count read back (["Accessibility on Windows"](#accessibility-on-windows-the-counter-through-ui-automation--2026-09-24), `cargo xtask device windows-a11y`); real pointer and keyboard input through `SendInput` — a missed click changes nothing, a click presses the button, Tab moves the focus to it as UIA reports, Enter presses it (["Input on Windows"](#input-on-windows-the-counter-through-sendinput--2026-09-24), `cargo xtask device windows-input`); Clippy for `x86_64-pc-windows-msvc` in `cargo xtask cross-typecheck` | One host (Windows 11 Pro, x86-64), a manual run rather than a CI job; the UIA client is not a Narrator session (no announcements or cursor navigation); input covers one click target, Tab and Enter — no text entry, IME, resize, DPI change or lifecycle verification, and buttons report not keyboard-focusable until focused (ADR-0079, "Not implemented"); the "Window-independent owner turns" and "Resident main-window validation" sections note Windows show/worker paths as cross-compilation evidence only |
...
control (button and static text); the Linux AT-SPI adapter compiles under
`--features a11y` and is not exercised (Windows UIA is, in the next section); iOS
publishes no accessibility tree at all (its row says so).

## Accessibility on Windows: the counter through UI Automation — 2026-09-24

`cargo xtask device windows-a11y` builds `examples/a11y_probe.rs` (release,
`--features material,a11y`), runs it on a real window, and is itself the
assistive technology: a UI Automation client, the API Narrator, NVDA and JAWS
read a window through. It finds the window by the probe's process id, walks the
raw UIA view, requires the prompt, the count and the button under the names a
screen reader speaks, invokes the button through `IUIAutomationInvokePattern`,
and waits for the count's name to become "1". No pointer or keyboard event is
synthesised. The oracle is names, not presence.

The first run found the defect that choice exists for: the tree held
`Text name=""`, `Text name=""`, `Button name="Increment"`. A `Text` reached
AccessKit as `Role::Label` with its text only in `label`, and the UIA and AT-SPI
adapters take a label node's name from its value
...
times:"`, `Text "0"`, `Button "Increment"` (plus the title bar's system menu and
caption buttons); after it `Text "1"` — `A11Y=PASS`. The same run with the fix
disabled prints both texts with empty names and exits 1. Limits: one host, a
manual check rather than a CI job; one client, not a Narrator session; one
control and two texts.

## Input on Windows: the counter through SendInput — 2026-09-24

`cargo xtask device windows-input` runs the same `a11y_probe` and drives it with
`SendInput` — events entering the system input queue as a device's would —
while UI Automation only locates the button and reads the result. Before every
event the probe's window must be the foreground window, so no keystroke can
reach another application. A click inside the window but off the button must
leave the count at "0"; a click on the button must make it "1"; Tab must give the
button the keyboard focus as UIA reports it; Enter must make the count "2".

The first run passed the click and failed the keyboard, and the check's
intermediate oracles took the failure apart into four defects, fixed together
(ADR-0079): no control answered Enter; an intent resolved at the `Shortcuts`
widget rather than the focused one; with nothing focused every key was dropped
before reaching the default bindings; and focus was not published to assistive
technology — no node was focused, and the root went out as a container that
AccessKit drops once the focus leaves it, after which UIA stopped walking the
window past its first child.

Accepted run (Windows 11 Pro 10.0.26200, x86-64): `the click pressed the
button: count 1`, `Tab focused the button; press Enter`, `Tab then Enter
activated the button: count 2`, `INPUT=PASS`; `windows-a11y` still passes on the
same build. Limits: one host, a manual check; one button; no text entry or IME
(no Japanese input method is installed on this host).
