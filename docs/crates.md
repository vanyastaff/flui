...
- the kind rule's: `flui-testing → flui-devtools` (a dev edge, the observation-seam test) until ADR-0088; the official packages not yet on the SDK, `flui-cupertino` → `flui-animation`, `flui-foundation`, `flui-objects`, `flui-types`, `flui-view`, `flui-widgets` and `flui-devtools` → `flui-foundation`, `flui-scheduler` until ADR-0088, and `flui-hot-reload` → `flui-foundation`, `flui-layer`, `flui-rendering`, `flui-view` until ADR-0094.
...
| `flui-sdk` | ✅ ACTIVE | The package-author surface of [ADR-0088](adr/ADR-0088-official-packages-sdk-and-facade.md): whole-module re-exports (`animation`, `foundation`, `types`, `view`, `widgets`) and curated `interaction`/`painting`/`rendering` subsets at the facade's paths, plus the Evolving `pipeline` module; the same items as the facade's, never wrappers. Evolving, versioned `0.N` apart from the train. Its normal graph reaches no host, engine or GPU crate. `flui-material` builds on it alone; `flui-cupertino` moves onto it next. The FLUI derives resolve through it first, so a package on the SDK alone can use the view, inherited and animation derives; the `Diagnosticable` derive has no SDK path yet. The train guard it relies on is `flui-foundation`'s `links = "flui_train"`. |
