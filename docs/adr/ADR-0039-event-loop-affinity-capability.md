...
2. **Realm dispatch already rejects wrong threads.** `RealmDispatcher { owner_thread, realm_id }` (`runner/realm_dispatch.rs`'s `RealmDispatcher`) and `dispatch_platform_realm`'s first check — `RealmDispatchError::WrongThread` — mean everything reached via realm dispatch already runs on the owner thread. Desktop bootstrap `debug_assert`s the same (`runner/desktop.rs`'s `debug_assert_eq!` on the owner thread).
...
- **Several first-party flows create windows before `run()` — or without any `run()` at all.** Pre-run: Android bootstrap (`runner/android.rs`'s `run_android` pre-run bootstrap), web bootstrap (`runner/web.rs`'s `run_web` bootstrap, "before run() since run() takes ownership"), `run_direct` (`direct.rs:101` — its own comment already documents it as broken on winit and needing the on_ready reorder `run_desktop` received), and the native Win32 examples (`examples/windows11_demo.rs:54`, `windows11_features.rs:77`, `test_background.rs:36`, `window_features.rs:59`). Run-less: headless tests construct `headless_platform()` and open windows with no event loop ever started (`binding.rs:3997`, `ui_realm.rs:748`, `runner/mod.rs`'s run-less headless sites). All run on the thread that owns (or would own) the loop; the design must give each a home, not delete their capability (see slices 2–3).
  - *Note (2026-09-22):* `windows11_features.rs` and `test_background.rs` were later removed as dead examples (nothing ran, referenced or documented them; PR #1262). `windows11_demo.rs` and `window_features.rs` remain and still show the Win32 flow. No test pins the Win32 ordering itself: `WindowsPlatform::run` calls `on_ready` on the owner thread before its message loop (`platforms/windows/platform.rs`, `fn run`), and flui-platform's tests do not run on Windows (H9). The same pre-loop, same-thread `on_ready` contract is exercised on the headless backend by flui-app's `owner_platform_host_installs_and_clears_around_run` (`app/runner/mod.rs`).
...
| `clipboard()` (`:232`) | resolution stays callable anywhere per ADR-0034's bootstrap shapes (Android/web resolve it pre-`run()`, `runner/android.rs` and `runner/web.rs` resolve it pre-`run()`); see §5 |
...
**Deleted outright in slice 3, not moved — landed 2026-09-07:** `hide`, `hide_other_apps`, `unhide_other_apps`, `should_auto_hide_scrollbars`, `window_stack`. The first four were default bodies in `traits/platform.rs` with zero backend overrides and zero callers; `window_stack` had zero consumers and no real-OS implementation — the trait default was `None`, winit returned `None` ("not easily supported"), macOS/Windows/Android inherited the default, and only the headless double and the web backend returned toy values, so its only implementations were the ones that could not fail.

> This is the one part of slice 3 that is **not** gated on the Android on-device validation below: nothing moves, five dead bodies are removed, and Android's bootstrap path is untouched. It shipped on its own for that reason. Moving the §1 method list off the trait still waits on that validation. ADR-0034's "What is deferred" discipline — no surface ahead of a real implementation *and* consumer — says these die rather than ride along; each returns (on `OwnerPlatform` if owner-affine) with its first real implementation and consumer.
...
- **Command ordering.** One bounded FIFO per owner; drained with a pre-read snapshot (`begin_drain`, `control.rs:177-183`) so a drain is finite and FIFO per batch (ADR-0027 §4). One producer's requests are observed in program order. `Quit` is deliberately *un*ordered: it is a flag riding any pending wake (`control.rs:149-157`, proven non-starvable when the queue is full by the existing lane test) and may overtake queued window requests — shutdown must not wait behind backpressure. A posted programmatic window close (`request_close_window`, the per-window list beside the flags, issue #919) is the one thing a quit does **not** overtake: it is lifecycle traffic already decided, never backpressured, and the owner runs every posted close's teardown (`on_close`, map removal, callback clear) before it exits — a quit that stranded a hidden-but-tracked window would reproduce the defect the close route exists to fix.
...
- **The host slot is loop-scoped, not realm-scoped.** `OwnerPlatform` lives in a new owner-thread TLS slot in `flui-app` (`OWNER_PLATFORM_HOST`), *separate from* the realm host (`PLATFORM_REALM_HOST`, since absorbed into `AppRuntime`, `app/runtime.rs`): installed at `on_ready` entry, cleared after `Platform::run` returns on backends where it returns (winit, headless, Android); on macOS `run` never returns (`terminate:` exits the process), so clearing is moot. It is deliberately **not** cleared in `teardown_platform_realm` (`runner/mod.rs`) — a realm's teardown must not strand the loop's capability, because the loop may host another realm before it exits (hot-restart does exactly this today, `install_platform_realm`, `runner/realm_dispatch.rs`). It cannot live in `AppBinding` fields without stripping `AppBinding`'s auto-traits.
...
- Because `dispatch_platform_realm` rejects wrong threads (`runner/realm_dispatch.rs`'s `dispatch_platform_realm`), every realm callback already executes where the capability lives; realm code uses it without any check — the `!Send` bound makes "checked at compile time" literal.
...
- **Multi-realm hosting on one owner thread** — the loop-scoped `OWNER_PLATFORM_HOST` removes this ADR's obstacle, but the realm host itself is single-realm today (since absorbed into `AppRuntime`, `app/runtime.rs`); hosting several realms per loop is its own design (ADR-0027 §1 policy surface).
