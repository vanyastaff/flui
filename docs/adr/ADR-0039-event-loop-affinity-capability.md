...
> **Update (2026-09-05) — the CI half of this statement no longer holds.** `flui-platform`'s Linux-runnable suite *does* run in CI: `ci.yml`'s `test` job carries a dedicated flui-platform step (`cargo nextest run -p flui-platform --all-features`) under two green-by-construction devices — `FLUI_HEADLESS=1` routing `current_platform()` to the mock, and `xvfb-run` giving the winit-internals tests a real X11 connection. So the assert wiring in that crate *is* executed by CI on Linux, and the paragraph above understates what is verified.
>
> The gap closed by a different route than this ADR anticipated, which is why the deferral outlived its reason: STATUS_HEAP_CORRUPTION is a **Windows-only** crash and never blocked the Linux subset, so re-inclusion did not have to wait on that investigation at all. What remains uncovered is unchanged and still accurately described above — the Win32, AppKit, and Android backends are never linked or executed anywhere; `cross-typecheck` lints them without linking, which is the only coverage they get.
