...
- **Widget-layer traversal geometry — discovered gap (2026-07-10 audit):** `FocusNode::set_rect` (`focus_scope.rs:320`) has **no production caller** — only the node-layer tests set rects by hand. A node mounted through the `Focus` widget therefore has a zero rect, and `ReadingOrderPolicy`'s sort degenerates to attach order (stable sort over equal keys). Tab order through widgets is deterministic but **not** reading order. H4's "done" is real at the node layer and untrue end-to-end; fixing it needs post-layout geometry fed into the node (the `AnchoredBox`/`PipelineOwner::box_size` pattern Hero uses) — a small seam, but a seam. Named here so the next traversal consumer starts from the truth.
