...
- **`FocusManager`** (`routing/focus.rs:126-507`): a process-global `OnceLock` singleton (`FocusManager::global()`) holding `root_scope`, `primary_focus`, focus-change listeners, per-node and global key handlers, and `active_scope` (the modal override). `AppBinding::handle_input` already routes every `PlatformInput::Keyboard` event into `FocusManager::global().dispatch_key_event` (`app/ui_realm/`'s `dispatch_key_event`).
...
- **Widget-layer traversal geometry — discovered in audit, closed same day (2026-07-10):** `FocusNode::set_rect` had no production caller, so widget-mounted nodes traversed in attach order (zero rects, stable sort). Closed with a **lazy `RectProvider`**: `FocusNode::rect()` consults an installed provider before the stored value; every `Focus` (and `EditableText`) anchors its child in an `AnchoredBox` and installs a provider measuring the anchor at traversal time — `box_size` + `transform_to` the render root, the `HeroHandle::bounding_box_in` shape, cleared on dispose so an external node never measures a dead anchor. Red-checked: skipping the install degenerates the three-node non-rotation test back to attach order.
