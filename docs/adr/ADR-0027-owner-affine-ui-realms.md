...
2. **Widgets singleton extraction + bound drop (coupled — not independently shippable):** `WidgetsBinding` leaves `OnceLock` storage into realm ownership *together with* dropping `Send + Sync` from `View`/`BuildContext`/`ViewState`/callback aliases — the static storage is what forces the bounds (`BindingBase` at `binding.rs:106`), so the two move as one landing. GlobalKey registry becomes realm-local in the same step.
