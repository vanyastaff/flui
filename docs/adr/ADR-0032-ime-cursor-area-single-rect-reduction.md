...
### 6. The inner anchor (ALT-2): a second `SubtreeAnchor` at the editable
...
- **A `flui-app`-level, full-widget-tree end-to-end geometry test.** `AppBinding::attach_root_widget`'s `RootRenderElement` bootstrap does not connect a mounted subtree's own render root as a child of its synthetic `RenderViewAdapter` node — confirmed independent of this feature (a bare `LeafView` shows the identical disconnect) and already tracked by the `PORT-TARGET` note above `AppBinding::attach_root_widget` (the by-id `ElementTree` vs. the runner's own hand-rolled root wiring — two coexisting ownership models). `transform_to` from a real mounted `EditableText` up to `root_id()` therefore cannot succeed through that specific bootstrap path today, regardless of the IME cursor-area wiring. The `flui-app`-level test in this change (`set_ime_cursor_area_reaches_the_active_windows_platform_capability`) instead exercises the `AppBinding -> TextInputPlatformBridge -> PlatformTextInput` plumbing directly, matching `perform_haptic_feedback_reaches_the_active_windows_platform_capability`'s shape; the geometry/dedupe/lifecycle coverage lives in `flui-widgets`' own test harness, which mounts through a connectivity-correct path (`tree.mount_root_with_pipeline_owner`, no synthetic root adapter) and does not depend on the pre-existing gap.
...
- Red-checks run (not merely asserted): dropping the alive-flag guard fails the blur-stop and dispose-stop tests; skipping `transform_rect` (sending the untransformed local rect) fails the ancestor-translation assertion; sharing `last_sent` across attaches fails the refocus-at-same-position half of the dedupe test; the visibility-independence contract is red-checked directly at the `RenderEditable` harness level (`harness_editable_caret_local_rect_is_visibility_independent`, paired with `harness_editable_hidden_caret_paints_no_caret_rect` as the paint-gating contrast).
