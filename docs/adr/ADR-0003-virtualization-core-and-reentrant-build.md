...
- **The deferred-mutation queue** (`crates/flui-rendering/src/pipeline/deferred.rs` + `PipelineOwner::apply_deferred_mutation`, `crates/flui-rendering/src/pipeline/owner/`; the deferred-mutation mechanism named here has since been removed, drained at the *end* of `run_layout`) is fully wired, but it is **next-frame materialization**: an `Insert` yields its `RenderId` at apply-time, *after* layout has finished — it is **not** mid-pass re-entrancy. It is, however, a sound, borrow-safe insertion path that can serve as a v1 build backend (see Decision 2).
...
- **v1 backend may be next-frame as a stepping stone.** The v1 *implementation* behind the contract may use the existing **next-frame deferred-mutation queue** (`pipeline/deferred.rs` + `apply_deferred_mutation`) **plus cache-region prefetch** (RecyclerView/Flutter-style: build the cache band ahead of need so the one-frame insertion latency is hidden by the buffer). This is a legitimate stepping stone — *but the contract stays mid-pass-shaped*, and **true mid-pass is a planned, recorded implementation unit (see the plan's U2 and U4), not an abandoned aspiration.**
  - **U3c completion note (2026-06-13).** The next-frame backend as first shipped (U3a) was *insufficient for a real consumer*: it dropped the logical item index on build (the freshly-inserted child's `SliverMultiBoxAdaptorParentData.index` defaulted to 0, so logical↔dense reconciliation never converged) and had no dispose path (no deferred-*remove* sink reachable from `perform_layout`, so the child list grew unboundedly). **U3c closes the contract**: the build request now carries the logical index **end-to-end** (request → `PendingBuild` → `DeferredMutation::Insert` → stamp via the `pub(crate) LogicalIndexParentData` trait at insert-apply), and a **symmetric deferred-remove sink** (`pending_removes`, drained Remove-before-Insert) pairs build with dispose so the band stays bounded. The mid-pass *shape* is unchanged — `ChildLayout`/`BoxChildRef` untouched — so a future true-mid-pass backend still slots in without a breaking change.
...
- Next-frame insertion path (v1 build backend candidate): `crates/flui-rendering/src/pipeline/deferred.rs`; `PipelineOwner::apply_deferred_mutation` at `crates/flui-rendering/src/pipeline/owner/`; the deferred-mutation mechanism named here has since been removed (drained at end of `run_layout`).
