...
Repair runs **before** the report is recorded, and it restores two things, not one: any parent still listing the contested child that is not its real parent has that edge dropped, and that parent's remaining children are restamped so each `ElementNode::slot` again matches its index in `child_ids`. A dangling child edge makes teardown cascade secondary failures on top of the real one, which is the same reason Flutter calls `forgetChild` there; Flutter needs no restamp because its child links are per-element fields rather than an index-addressed list.

The restamp is the half that actually fires. The production graft unlinks the child from its old parent on the way past, so by report time the edge is usually already gone — but that removal shifts every later sibling down one index without touching its slot, and slot is read by reconcile-event dispositions, tree observers, and render-child ordering long before the parent's next rebuild would restamp them. Restamping is idempotent and runs only for parents named in a report, so an ordinary frame never pays for it.
...
- **Keep hashing, document the collision risk.** This is what the code did, with a comment on the registration path acknowledging the collision policy outright. It cannot be made correct by documentation: the retake path reads the map, so a collision is a state transplant, not a lookup miss. And it leaves the duplicate check unable to distinguish the two cases it exists to separate.
