...
**Diagnostics composes *down to the leaf value types*, at the right layer.** The geometry/colour value types (`Rect`, `Color`, `Point`/`Offset`, `Size`, `RRect`, `Matrix4`) get `impl From<T> for DiagnosticsValue` **in `flui-foundation`** — legal under the orphan rule because `DiagnosticsValue` is the local type and `flui-foundation` already depends on `flui-types`/`flui-geometry`. A generic `DiagnosticsBuilder::add_value(name, impl Into<DiagnosticsValue>)` then lets every `Diagnosticable` impl write `add_value("rect", rect)` instead of destructuring fields at each call-site. The conversions live one layer *above* the types (where the diagnostics vocabulary lives) and reach *down* to them — so the introspection vocabulary covers the whole type system **without** the rejected alternative of `flui-types` depending on the diagnostics crate, which would be a dependency cycle (`flui-foundation` already depends on `flui-types`). Value types convert to a `DiagnosticsValue` (a `Rect`-as-property is `DiagnosticsValue::Rect`); only *objects* (RenderObject/Layer/DrawCommand) implement `Diagnosticable` (→ a node). (Plan Task 4.5.)
