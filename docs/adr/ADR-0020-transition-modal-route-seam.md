...
| The push-deferral seam | `PushCompletion::Animating(TickerFuture)` awaited by `NavigatorShared::apply` — the navigator wiring for #1161, per [ADR-0064](ADR-0064-animation-completion-is-one-controller-resolved-future.md); `RouteHistory::notify_push_completed` is gone |
