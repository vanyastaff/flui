...
`Container` satisfies both: every level it absorbs (`Align`, `Padding`, `ColoredBox`, `DecoratedBox`, `ConstrainedBox`, `Transform`) is geometry/paint over the same caller-supplied child, none of them builds or owns parent-data, and the whole point is the reconciliation hazard in (a). `SafeArea` fails (a): it is fixed-depth (always exactly one `Padding`), so no level ever appears or disappears under the caller — there is no identity hazard to fix by collapsing it. `Card`, `ListTile`, and `Material` fail (b): they build child widgets (icons, text, ink effects) and are not reducible to box-protocol geometry/paint over the caller's child, so composing them is not this exception — it is the ordinary case §Decision already describes.
