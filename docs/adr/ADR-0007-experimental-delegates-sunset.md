...
## Amendment (2026-07-01): `FlowDelegate` un-gated — sunset cancelled for that module

`RenderFlow` landed in `crates/flui-objects/src/layout/flow.rs`, with the `Flow` widget in `crates/flui-widgets/src/layout/flow.rs`. Per this ADR's own rule ("one landing render-object proves the pattern is active"), this satisfies the sunset-cancelling condition for `delegates/flow_delegate.rs` specifically — the same event that already cancelled it for `delegates/sliver_grid_delegate.rs` (when `RenderSliverGrid` landed) and `delegates/custom_painter.rs` (when `RenderCustomPaint` landed, see the amendment above).

**Consequence:** `delegates/flow_delegate.rs` is un-gated (moved out of `#[cfg(feature = "experimental-delegates")]` in `delegates/mod.rs`) and ships unconditionally, alongside `sliver_grid_delegate.rs` and `custom_painter.rs`. `FlowPaintingContext` was also redesigned while making this change: it now carries the transform passed to `paint_child` through to either a live `PaintCx` (real paint) or a recording-only replay context (hit-test), instead of discarding it — the previous shape recorded only a `painted: Vec<bool>` flag and never let a transform escape the context, which meant no real `RenderFlow` could have been built on it as-is. `paint_child` also gained the oracle's double-paint assert (`RenderFlow.paintChild`, `flow.dart` L356-365), which the pre-redesign context was missing entirely.

**Still gated, unchanged by this amendment:** `delegates/multi_child_layout_delegate.rs` and `delegates/custom_clipper.rs` — neither companion render object (`RenderCustomMultiChildLayoutBox`, the clip-delegate consumers) has landed. `delegates/single_child_layout_delegate.rs` was un-gated separately once `RenderCustomSingleChildLayoutBox` landed (see `delegates/mod.rs`'s current feature-gating doc comment for the up-to-date list). The sunset trigger in the Decision section above still applies to the two modules named here.

---

## Amendment (2026-07-01): `SingleChildLayoutDelegate` un-gated — sunset cancelled for that module

`RenderCustomSingleChildLayoutBox` landed in `crates/flui-objects/src/layout/custom_single_child_layout.rs`, with the `CustomSingleChildLayout` widget in `crates/flui-widgets/src/layout/custom_single_child_layout.rs`. Per this ADR's own rule, this satisfies the sunset-cancelling condition for `delegates/single_child_layout_delegate.rs`.

**Consequence:** `delegates/single_child_layout_delegate.rs` is un-gated and ships unconditionally, alongside `sliver_grid_delegate.rs`, `custom_painter.rs`, and `flow_delegate.rs`. The delegate trait now also carries Flutter's default implementations for `get_size`, `get_constraints_for_child`, and `get_position_for_child`; `should_relayout` remains required.

**Still gated, unchanged by this amendment:** `delegates/multi_child_layout_delegate.rs` and `delegates/custom_clipper.rs` — neither companion render object (`RenderCustomMultiChildLayoutBox`, the clip-delegate consumers) has landed.

---

## Amendment (2026-07-01): `MultiChildLayoutDelegate` un-gated — sunset cancelled for that module

`RenderCustomMultiChildLayoutBox` landed in `crates/flui-objects/src/layout/custom_multi_child_layout.rs`, with the `CustomMultiChildLayout` and `LayoutId` widgets in `crates/flui-widgets/src/layout/custom_multi_child_layout.rs`. Per this ADR's own rule, this satisfies the sunset-cancelling condition for `delegates/multi_child_layout_delegate.rs`.

**Consequence:** `delegates/multi_child_layout_delegate.rs` is un-gated and ships unconditionally, alongside `sliver_grid_delegate.rs`, `custom_painter.rs`, `flow_delegate.rs`, and `single_child_layout_delegate.rs`. The delegate trait now also carries Flutter's default `get_size -> constraints.biggest()`; `perform_layout`, `should_relayout`, and `as_any` remain required.

**Still gated, unchanged by this amendment:** `delegates/custom_clipper.rs` — the clip-delegate consumers have not landed.
