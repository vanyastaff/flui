...
| ~~**Named-route generation on the navigator**~~ — `on_generate_route` / `on_unknown_route` / a route table / `push_named` / `RouteKey<T>`: **landed 2026-09-07** on `NavigatorHandle`, see [ADR-0024](ADR-0024-named-routes-seam.md) §7. | — | Registration is on the handle, not the widget. |
| **App-level route table** (`WidgetsApp::routes(..)` / `MaterialApp::routes(..)` forwarding into the navigator's registry) | None technical. | **Still deferred.** The mechanism exists; nothing forwards to it, so there is no app-level `routes` map to call. Do not read the row above as delivering one. |
| **Initial-route synthesis** (`initialRoute` / `defaultGenerateInitialRoutes` back-stack expansion) | None technical. | **Still deferred** — ADR-0024 **U3**, whose obligations are recorded in `crates/flui-widgets/src/navigator/mod.rs`. |
...
**Update (2026-07-10, later):** `PopScope` is public too, giving `RoutePopDisposition::DoNotPop` its producer. The shape is Flutter's `PopEntry` machinery verbatim (`routes.dart:1980`, `:2033-2050`): `ModalInner` owns a `PopEntryRegistry` provided to the page as an ambient scope (the `HeroScope` pattern); the `Route` trait gained `vetoes_pop()` (checked in `pop_disposition_of_top` **before** `will_handle_pop_internally`, matching `ModalRoute.popDisposition` over the `LocalHistoryRoute` layer); and `Route::on_pop_invoked` fans out to every registered scope. `can_pop(false)` blocks `maybe_pop` only — a programmatic `pop()` still pops, as in Flutter. The first test run caught the `PageRoute`→`ModalRoute` delegation missing the new trait method (the default `false` silently disabled the veto) — the forward is now explicit, and the end-to-end test pins veto, refusal notification, pop-through, and deregistration-on-dispose. Still deferred: `onPopInvokedWithResult`'s result payload, `NavigationNotification`, and page-based `canPop`.
