...
- `ScaffoldScope::maybe_of`'s consumers read the `DrawerHandle` and use it immediately, or hold it as a stable capability object whose *own* methods read live state — nothing needs to be told "the ancestor `Scaffold` changed identity, go re-read `Overlay::maybe_of`" (renamed for this ADR's example, but the shape is `ScaffoldScope`'s).
