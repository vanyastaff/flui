...
*`Overlay`, `OverlayEntry`, `OverlayEntryId`, and `OverlayHandle` are published from `flui-widgets`' crate root. `Overlay::of`/`Overlay::maybe_of` resolve a new `OverlayScope` — an `InheritedView` marker mounted **per entry**, wrapping that entry's built child — rather than a `findAncestorStateOfType`-style walk to a shared `OverlayState`. This mirrors the 3.44.0 oracle's own marker-per-entry mechanism, not an earlier one. It is a `depend_on` (dependency-registering) lookup, not a lookup-only `get` — but that is a **FLUI-native divergence**, not oracle parity: the 3.44.0 oracle's `Overlay.maybeOf` explicitly passes `createDependency: false`, so neither oracle entry point registers a dependency at all. `depend_on` is used anyway because `DraggableState` — the one production consumer this unlocks — has to cache the resolved handle ahead of time for a `BuildContext`-free gesture callback, and needs the dependency to keep that cache honest if the ancestor overlay ever changes.*
...
Most of `flui-widgets` tracks Flutter tag `3.33.0-0.0.pre-6280-g88e87cd963f` (see `overlay/mod.rs`'s own parity header). Checked directly against `3.44.0`'s `widgets/overlay.dart`: modern `Overlay.maybeOf` resolves `_RenderTheaterMarker` — a private `InheritedWidget` that `_OverlayEntryWidgetState.build` mounts around **each entry's own child** — via `_RenderTheaterMarker.maybeOf(context, createDependency: false)`. `_RenderTheaterMarker.maybeOf` itself defaults `createDependency` to `true`, but `Overlay.maybeOf` (`overlay.dart` lines 634-639) overrides it to `false` explicitly, and `Overlay.of` routes through `maybeOf` — so **neither oracle entry point ever registers a dependency**. Older Flutter releases instead used `context.findAncestorStateOfType<OverlayState>()`: a plain ancestor walk to one shared `State`, also with no dependency, just no per-entry granularity either.

The per-entry marker is still worth porting on its own terms — nested overlays need it regardless of the dependency question (see Decision §2). But the dependency-registering `depend_on` call this ADR ships is **not** loyal porting of `Overlay.maybeOf`; it is a deliberate FLUI-native addition, ruled on in Decision §3 below.

## Decision

### 1. Publish four types; keep the mutation surface private

> **Superseded in part by [ADR-0076](ADR-0076-public-overlay-mutation-api.md):** the mutation surface this section keeps `pub(crate)` is public API from ADR-0076 on. The four published types and §2–§4 stand.
...
### 3. `depend_on`, not `get` — a FLUI-native divergence, ruled on its own merits

`Overlay::maybe_of` resolves via `BuildContextExt::depend_on` (which registers a dependency), not the lookup-only `get` that `crate::ScaffoldScope::maybe_of` (`flui-material`) uses for its `DrawerHandle`. **This is not the oracle's own choice** — corrected from an earlier draft of this ADR, which claimed `Overlay.maybeOf` registers a dependency: it does not (see "The oracle mechanism," above — `Overlay.maybeOf` passes `createDependency: false` explicitly). A dependency-free `get` would in fact be the *loyal* port of `Overlay.of`/`maybeOf`.

`depend_on` is used anyway, and the ruling is FLUI-native, not oracle-derived: what distinguishes `Overlay::maybe_of` from `ScaffoldScope::maybe_of` is not "which one the oracle contracts a dependency for" (neither does) but **which one has a consumer that needs re-resolution**:

- `ScaffoldScope::maybe_of`'s consumers read the `DrawerHandle` and use it immediately, or hold it as a stable capability object whose *own* methods read live state — nothing needs to be told "the ancestor `Scaffold` changed identity, go re-read `ScaffoldScope::maybe_of`".
- `Overlay::maybe_of`'s first (and so far only) production consumer, `DraggableState` (`interaction/draggable.rs`), is different in a way that is specific to FLUI's architecture: the thing that would actually need the overlay mid-drag is `DragSession`, which implements `MultiDragHandle` — a trait bounded `Send + Sync + 'static` — and holds **no `BuildContext`** at all. Flutter's `_DragAvatar.update` can call `Overlay.of(context)` fresh, on demand, because Dart closures keep `context` alive for free and a plain ancestor lookup is cheap to repeat. FLUI's session object structurally cannot do that: the handle has to be resolved and cached *ahead of time*, in `DraggableState::init_state`/`did_change_dependencies` (a lifecycle hook, per ADR-0018's pattern — see Decision §4), for a gesture callback with no `BuildContext` to read later. `depend_on` is what keeps that cached value honest if a *different* overlay identity ever replaces the resolved one; `get`, resolved once and never re-checked, would let the cache go silently stale the first time it needed to change.
