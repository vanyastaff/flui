...
|---|---|---|---|---|
| N1 | **D-1 layout phase wired** — `run_layout` → `layout_dirty_root` invokes per-node `RenderEntry::layout` with constraints propagated parent→child (replaced `layout_node_with_children`, now deleted; `OnceCell`→`Option` state migration done) | ✓ done | [`plans/2026-05-23-001-feat-pipeline-wiring-d-block-plan.md`](plans/2026-05-23-001-feat-pipeline-wiring-d-block-plan.md) | Impl: `crates/flui-rendering/src/pipeline/owner/layout.rs`. Gate test `crates/flui-rendering/tests/u23_run_layout_wiring.rs` (Padding→Center→ColoredBox sizing) — `cargo test -p flui-rendering --test u23_run_layout_wiring` 6/6 pass (2026-06-30) |
| N2 | **D-3 `run_compositing`** — subtree compositing-bits walk implemented (Flutter `_updateSubtreeCompositingBits`) | ✓ done | same plan as N1 | Impl: `crates/flui-rendering/src/pipeline/owner/compositing.rs`. Gate test `crates/flui-rendering/tests/u34_compositing_bits_walk.rs` — `cargo test -p flui-rendering --test u34_compositing_bits_walk` 7/7 pass (2026-06-30) |
| N3 | **D-4 `run_paint` dirty-flag fix** — clear `needs_paint` only on nodes actually painted (`painted`/`dirty_ids` set; unreached nodes warn+clear, R-15) | ✓ done | same plan as N1 | Impl: `crates/flui-rendering/src/pipeline/owner/paint.rs`. Gate test `crates/flui-rendering/tests/u35_paint_dirty_flag_discipline.rs` (incl. `u35_repaint_boundary_isolates_subtree_paint`) — `cargo test -p flui-rendering --test u35_paint_dirty_flag_discipline` 5/5 pass (2026-06-30) |
...
| N6 | **Refusal triggers #8–#13** installed in [`PORT.md`](PORT.md); mechanically-detectable ones become `port-check.sh` gates | ✓ done | [`scripts/port-check.sh`](../scripts/port-check.sh) | `bash scripts/port-check.sh -v` exits 0 — all **20** triggers + FR-033 green, incl. #8 (SP-1), #10 (SP-3), #11 (SP-4), #12 (SP-6), #13 (SP-8). PORT.md documents SP-1/SP-3/SP-4/SP-6/SP-8 with D-block back-references (verified 2026-06-30) |
...
| N9 | **Constitution layer table + edition/Rust-version line amended** | 🛇 blocked | unblocked by a maintainer restoring `.specify/memory/constitution.md` (the file — and `.ai-factory/` — is absent from the repo, lost in a history squash) | Live docs are correct: [`FOUNDATIONS.md`](FOUNDATIONS.md) Part IV matches `Cargo.toml` (`edition = "2024"`, `rust-version = "1.96"`). But the constitution **artifact itself is missing**, so its version bump + layer-table sync cannot be recorded until it is restored. Done 2026-06-30: FOUNDATIONS `1.95`→`1.96` corrected, dangling `.specify/memory/constitution.md` references across live docs reconciled (point to FOUNDATIONS Part IV as de-facto layer-table source) |
...
| N15 | **`cargo build --workspace` + `cargo clippy --workspace --all-targets -- -D warnings` exit 0** (Core.0 final gate) | ✓ done | run `just ci` | Both exit 0 against `main` `0da97a4` (verified 2026-06-30). NOTE: local builds need `RUSTFLAGS="-C link-arg=-fuse-ld=lld"` to bypass a global-cargo-config mold-linker crash on `bon-macros`; CI uses its own linker and is unaffected |
