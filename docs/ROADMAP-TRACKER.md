...
| **PR 2 — Option D DEFAULT (per U17 spike, 2026-05-29) — pending user confirmation of the default-flip** ||||
| N-geom.U14 | **(DEFAULT — selected by U17)** Option D — wrap `Matrix4` / `Vec2<U>` / `Transform*` over `glam::Mat4`/`Vec2`/`Affine*` internals; preserve public API (incl. fields → 0 downstream churn) | ☐ **DEFAULT after U17** | research §VIII Option D + [U17 report](research/2026-05-29-u17-euclid-spike-report.md) | `#[repr(transparent)]` wrappers; `size_of::<Matrix4>() == size_of::<glam::Mat4>()` + `align_of` tests; **126 public fns** → **500–850 LOC delegation**; `inverse()` keeps `Option<Matrix4>`; hand-written `mul_simd_sse`/`mul_simd_neon` deleted; `simd` Cargo feature removed; `glam = { features = ["bytemuck", "mint"] }` + `kurbo = { features = ["mint"] }`; engine Pod-conversion shim deletable; deletes 3,833 LOC of own math behind the stable API |
