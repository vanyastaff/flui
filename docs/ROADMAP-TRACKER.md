...
**Exit:** widget→render-object checklist complete; per-RO layout + paint tests; intrinsic-size tests where applicable; 1000-item sliver scroll test green; `flui-rendering` coverage ≥ 80%. **Secondary-query gap CLOSED** (verified 2026-07-01 — `ADR-0010`/`ADR-0011`/`ADR-0012`, see the closure note above); 1000-item scroll coverage exists in multiple test files (`crates/flui-objects/tests/harness_snapshot.rs`, `crates/flui-rendering/tests/u3c_lazy_sliver_contract.rs`) but has not been re-audited end-to-end against this specific exit-gate wording in this pass. **`flui-rendering` coverage measured 2026-07-01 (`cargo llvm-cov -p flui-rendering --summary-only`): 75.50% lines / 78.09% regions / 72.58% functions — below the ≥80% target.** Not chased in this pass: a first look at the lowest-covered files shows the gap is a mix of genuine untested paths and measurement artifacts (e.g. `protocol/capabilities.rs` at 0% is a pure trait-definition file whose default method bodies are overridden by `BoxProtocol`/`SliverProtocol`, which are separately tested at ~77-80%/67% — chasing its 0% would mean testing trait mechanics, not product logic). Closing the real remainder of this gap needs its own file-by-file audit to separate genuine gaps from this kind of artifact, not a blind push toward the raw percentage.
