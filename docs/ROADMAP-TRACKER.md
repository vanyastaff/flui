...
| N5.1 | ↳ Phase 1 — storage shape + key field + self-validation round-trip tests | ☐ todo (UNBLOCKED — Phase 0 ✓) | (precise plan below, 2026-06-30) | `cargo test -p flui-view` green; element storage layout matches spec |

**N5.1 Phase 1 execution plan (scoped 2026-06-30 — atomic, no partial commit possible):**
...
- **The atomic change:**
  1. `ElementNode.element: Option<Box<dyn ElementBase>>` (`element_tree.rs:33`) → `kind: ElementKind`; update `ElementNode::new` (`:149`) + `put_element` (`:939`) signatures to take `ElementKind`.
  2. Change the `create_element() -> Box<dyn ElementBase>` trait method to **`-> ElementKind`**, then update all **52 impls across ~25 files** (`view/{stateless,stateful,proxy,inherited,render,animated,memo,parent_data,error,root}.rs`, `element/behavior_commons.rs`, `seq/{tuple_impls,vec_impls}.rs`, `child/`, `macros/mod.rs`, etc.) — each boxes to its specific sub-trait variant (`StatelessElementBase` → `ElementKind::Stateless`, render-by-arity → `RenderLeaf/Single/Optional/Variable`). NB: the four `Render*` variants have no Phase-1 blanket impl yet (per kind.rs doc) — confirm the render `create_element` path maps to `RenderVariable` (the only one with a concrete blanket impl) or extend as needed.
  3. Update the **29 `node.element` field reads** → `node.kind.element()` (read-bridge) or a typed `match` where the concrete behavior is needed.
- **No central conversion exists** — the variant is chosen *inside* each `create_element` impl (it holds the concrete `Element<V,A,B>` before boxing). This is why it's atomic and per-site.
...
- **Phase 2** (separate): keyed reconciler completion (FR-024/025) + `ElementCore` rewiring + `ReconcileEvent`. **Phase 3:** `IntoView` surface + derive macros + `downcast_ref` count→0 verification.

**↳ RESOLVED MECHANICAL RECIPE (reverse-engineered 2026-06-30 — design done, execution is now transcription):**
1. **`create_element` is macro-driven** — so the bulk is ~6 macro edits, not 52 hand-edits. Change `View::create_element(&self) -> Box<dyn ElementBase>` → `-> ElementKind` (`view/view.rs:58`), then update each macro body to wrap in its variant (the `Box::new(..)` coerces to the variant's sub-trait):
   - `impl_render_view!` (`view/render.rs:105`) → `ElementKind::RenderVariable(Box::new(RenderElement::new(self, RenderBehavior::new())))` — **render crux RESOLVED: `RenderElement<V> = Element<V, Variable, RenderBehavior<V>>` (`element/mod.rs:116`), i.e. ALL render elements are `Variable` arity and the only wired `RenderElementBase<_>` blanket impl is `Variable` (`kind.rs:160`) → every render view → `RenderVariable`. Leaf/Single/Optional variants stay reserved for Phase 2/3.**
   - proxy macro (`view/proxy.rs:56`) → `ElementKind::Proxy(Box::new(ProxyElement::new(self, ProxyBehavior)))`
   - inherited macro (`view/inherited.rs:107`) → `ElementKind::Inherited(Box::new(InheritedElement::new(self, InheritedBehavior::new(self))))`
   - stateless derive → `ElementKind::Stateless(Box::new(StatelessElement::new(self, StatelessBehavior)))`
   - stateful derive → `ElementKind::Stateful { element: Box::new(StatefulElement::new(self, StatefulBehavior::new(self))), animation_listener: None }`
   - memo / animated / parent_data macros → map to their family (animated → `Stateful` w/ listener; memo + parent_data → `Proxy`).
2. **Hand-written `View` impls** (incl. `#[cfg(test)]` `TestView`/`TestCounter`/dummy elements in stateless.rs/stateful.rs/proxy.rs/render.rs/inherited.rs, plus `view/root.rs`, `view/error.rs`, `macros/mod.rs`) → same wrapping per family.
3. **Storage** (`tree/element_tree.rs`): `element: Option<Box<dyn ElementBase>>` (`:33`) → `kind: Option<ElementKind>`; `ElementNode::new` (`:149`) takes `kind: ElementKind`, stores `Some(kind)`; `put_element`/`take_element` → `Option<ElementKind>`. Callers (`insert`/`mount_root_*`) pass `view.create_element()` directly (no `Box`).
4. **Accessors** (29 `node.element` reads): immutable → `node.kind.as_ref().map(|k| k.element())` (use the `ElementKind::element()` read-bridge at `kind.rs:316`). **Add an `element_mut(&mut self) -> &mut dyn ElementBase` bridge to `ElementKind`** (it currently only has `element()`); mutable accessors match on the variant via it.
5. **Imports**: add `ElementKind` (`$crate::element::ElementKind` in exported macros) to every touched file.
6. **Verify**: `cargo check -p flui-view` (drive the cascade) → `cargo check --workspace` → `cargo test -p flui-view` incl. `tests/key_roundtrip.rs` → clippy → port-check 21/21. Atomic: green-or-revert. Best run with a fresh full context budget; the cargo lock is shared with sibling projects so build when the machine is otherwise idle.

**✓✓ ALL PHASE 1 DESIGN BLOCKERS RESOLVED 2026-06-30 (commit `f61346ee`).** The `ElementKind` taxonomy is now complete — every element type maps to a variant: 5 behavior families + `Animated`→`Stateful` + `ParentData`→`Proxy` (`618acd70`) + **dedicated `Root` / `Error` variants** with sealed `RootElementBase`/`ErrorElementBase` sub-traits (`f61346ee`, allowlisted in port-check #9). Design rationale: root (owns `PipelineOwner`/bootstrap) and error-boundary leaf are first-class *kinds*, not behavior-family elements; dedicated sealed variants beat both forcing-onto-`Element<V,A,Behavior>` and a raw `Box<dyn ElementBase>` escape. **N5 Phase 1 is now PURE MECHANICAL TRANSCRIPTION** — no design unknowns remain; the create_element return-type sweep (trait + ~55 bodies + flui-macros derives + storage field + accessors, atomic) is all that's left. **Sweep note:** the `#[cfg(test)]` dummy elements (`DummyElement` in inherited.rs, `NopElement` in behavior_commons.rs, `DummyChildElement` in proxy.rs, `TestElement` in root.rs, `LeafElement`/`MultiNodeElement` in binding.rs) are standalone `ElementBase` structs with no variant — each must be refactored in the sweep to construct a *standard* element (e.g. a `StatelessElement` over a tiny test view) so its `create_element` can return an `ElementKind`. Mechanical but adds volume (~70–90 edits total); size + atomicity (no partial compile) are why this wants a fresh full-context pass, not the design — the design is done. The original blocker analysis is retained below for context.

**(resolved) ⛔ DEEPER BLOCKER — `ElementKind` variant set was INCOMPLETE for production elements (found 2026-06-30, the decisive one):** the premise that `ElementKind` is "ready scaffolding, just wire it" is **wrong**. `create_element` bodies produce element types that have **no `ElementKind` variant** because they are standalone `ElementBase` structs, NOT `Element<V,A,Behavior>` (which is all the 8 variants can hold):
- **`ErrorElement`** (`view/error.rs:218`, PRODUCTION — the C7 catch_unwind error boundary) — standalone struct, no variant.
- **`RootRenderElement`** (`view/root.rs:93`, PRODUCTION — the render-tree root) — standalone struct, no variant.
- Test-only: `LeafElement`/`MultiNodeElement` (`binding.rs`), `NopElement` (`behavior_commons.rs`), `DummyElement`/`DummyChildElement`/`TestElement` — also non-unified; need refactor-to-standard-behavior or a test-only path.
- Also: production **`Leaf`-arity render elements** would need `ElementKind::RenderLeaf` (+ `Single`/`Optional`), whose blanket impls `kind.rs:152-159` defers to Phase 2/3.

**CONSEQUENCE — Phase 1 is NOT mechanical transcription; it is real `ElementKind` DESIGN work.** Full analysis of the two production blockers (2026-06-30):
- **`ErrorElement`** (`view/error.rs:218`) — a **leaf element that renders directly** (10 `ElementBase` methods, no children). Cannot become a `StatelessView` (flui-view has no widgets for it to `build()`). Options: (a) build the deferred `RenderLeaf` blanket impl + make `ErrorView` a leaf `RenderView` → `ElementKind::RenderLeaf`; (b) a dedicated `ElementKind` variant. (a) is the cleaner long-term shape (unifies leaf-render) but pulls in the Phase 2/3 render-arity work.
- **`RootRenderElement`** (`view/root.rs:93`) — genuinely special: owns the `PipelineOwner`, root `RenderId`, render-tree bootstrap, and a root-only `needs_build` dirty bit (Flutter's `_RawViewElement`/`RenderTreeRootElement`). NOT an `Element<V,A,Behavior>`; refactoring onto one would entangle pipeline bootstrap. Options: (a) a dedicated `ElementKind::Root` variant; (b) keep the root OUTSIDE `ElementKind` (the root slab entry is special-cased — it is mounted via `mount_root_*`, distinct from `insert`). (b) is likely cleaner — the root is already a distinct mount path.

**Recommended direction for the design session:** `RootRenderElement` → keep outside `ElementKind` (special root mount path, option b); `ErrorElement` → dedicated minimal variant OR fold into `RenderLeaf` once that arity is wired. Decide with chief-architect / `/team-api`; this is the kind of call that must be *right* for "competitors applaud," not bashed out under a deadline. **This is the true reason Phase 1 needs a focused design session.** The earlier "pure transcription" framing was corrected by this finding.

---

**(earlier, partial) ✓ BLOCKER RESOLVED 2026-06-30 (commit `618acd70`):** added the two missing `ElementKind` blanket impls — `StatefulElementBase for Element<V, Single, AnimatedBehavior<V>>` (animated → `Stateful`) and `ProxyElementBase for Element<V, Single, ParentDataBehavior>` (parent-data → `Proxy`). All `create_element` families now map to a variant, so Phase 1 is pure mechanical transcription. **Animated `create_element` uses `ElementKind::Stateful { element, animation_listener: None }` in Phase 1** — the FR-020 `AnimationListener` capture lands in Phase 2/3 when `AnimatedBehavior` joins dispatch; `None` is the deliberate Phase-1 value, not a bug. Original blocker description retained below for context:

**(was) ⚠ BLOCKER (Phase 1 ↔ Phase 2 coupling — not in the original scope):** `ElementKind`'s blanket sub-trait impls cover only **5** behaviors (`element/kind.rs:120-160`): `StatelessBehavior`, `StatefulBehavior`, `ProxyBehavior`, `InheritedBehavior`, `RenderBehavior`(Variable). But two `create_element`-producing families have **no** `ElementKind` mapping:
- **`AnimatedElement<V> = Element<V, Single, AnimatedBehavior<V>>`** (`element/mod.rs:128`; macro `impl_animated_view!` `view/animated.rs:120`). `kind.rs:177` explicitly defers `AnimatedBehavior` dispatch to "a later phase," and the `ElementKind::Stateful { animation_listener }` field exists precisely to host it — but the *mechanism* (extract the `StatefulElementBase` view of an `AnimatedElement` + capture the `AnimationListener` closure at mount, FR-020) is unbuilt. **This is the hard part of N5 and is genuinely Phase 2/3 design, not Phase 1 transcription.**
- **`ParentDataElement<V> = Element<V, Single, ParentDataBehavior>`** (`element/mod.rs:137`; macro `impl_parent_data_view!` `view/parent_data.rs:160`). Likely resolvable cheaply by adding `impl ProxyElementBase for Element<V, Single, ParentDataBehavior>` (if `ParentDataBehavior` satisfies the proxy surface) → `ElementKind::Proxy`. Verify before relying on it.

**Consequence:** Phase 1 cannot be a pure mechanical transcription — it must FIRST add the missing blanket impls (cheap for `ParentData`→`Proxy`; **design-heavy for `Animated`→`Stateful`+`AnimationListener`**), or temporarily route animated/parent-data `create_element` through a documented fallback variant. Resolve this BEFORE starting the atomic edit, or animated.rs/parent_data.rs (+ their test impls) will not compile. This finding is why Phase 1 is not a tail-of-session grind.
