...
**Progress (2026-05-29):** **PR 1 landed** — U1/U2/U3/U4(+U9/U10)/U6/U7/U12 complete; unit barrier enforced by `port-check.sh` trigger #14. U5 + U6.1 deferred to PR 2 (rationale in rows below); U11 optional follow-up. Next: **U17 euclid spike** (risk gate before PR 2). Workspace builds + clippy `-D warnings` clean; the unit-barrier removal cascaded fixes through `flui-types`/`flui-rendering`/`flui-layer`/`flui-painting`/`flui-semantics`/`flui-app` (a `FloatUnit` bridge trait replaced the implicit `From<f32>` used by generic curve/shape math).
...
| N-geom.U5 | Deprecate `to_device_pixels(f32)` + wrapper cascade in Size/Point/Bounds | ☐ todo → **deferred to PR 2** | research §III U5 | Deferred: `#[deprecated]` would break `-D warnings` across ~80 `flui-types` test/example sites only for PR 2's `euclid::Scale` swap to delete the fns wholesale (research §"U7 collision check" confirms forward-compatible). Doc comments already steer callers to the typed `to_device(ScaleFactor)` path. |
...
| N-geom.U6.1 | **Delete `ScaledPixels` and all `Scaled*` aliases** (SP-4, decision in research §VIII DevicePixels representation) | ☐ todo → **deferred to PR 2** | research §VIII DevicePixels decision | Deferred to keep PR 1 atomic: `ScaledPixels` still has live re-exports + a `flui-types` example/README; the 2-tier collapse belongs with PR 2's unit-system reshape (research permits "PR 1 or PR 2"). |
