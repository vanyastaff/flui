...
| B1.5 | `flui-widgets` coverage ≥ 85% | ⚠ verify | Initial measurement 2026-07-01 (`cargo llvm-cov -p flui-widgets --no-fail-fast`, needed due to one order-dependent flaky test — confirmed pre-existing `flui-app` singleton-state flake per AGENTS.md's Testing Quirks, passes cleanly at `--test-threads 1`): 69.35% regions / 70.16% functions / 72.64% lines. Closed via a per-file audit (not blind percentage-chasing), one file/commit at a time: the five 0%-coverage files (`absorb_pointer.rs`, `decorated_box.rs`, `fitted_box.rs`, `sliver_opacity.rs`, `text_field.rs` — the last two via widget-mount tests using the `common::lay_out`/`Viewport` harness, including a novel `AbsorbPointer` hit-test-absorption test and `TextField`'s first real `build()`-mounting test); `flex.rs`/`stack.rs` (the plain `Flex`/`Stack` widgets and non-default alignment were untested, only `Row`/`Column`/`Positioned` had coverage); `semantics/mod.rs` (37%→99%, the largest single gap — `Semantics`'s ~28 chained builder methods had zero tests); `image/provider.rs` (14%→94% — the default-feature-reachable paths, distinct from the already-tested `images`-feature-gated decode path); `text/text.rs` (`max_lines`/`style`/`direction`, via comparative assertions robust to the font backend); and five small sliver proxy widgets (`sliver_list.rs`, `sliver_fixed_extent_list.rs`, `sliver_grid.rs`, `sliver_fill_viewport.rs`, `spacer.rs` — `Debug`/`has_children`/`update_render_object` field-level effects, verified via derived `Debug` output where no public getter exists). Two real Flutter-parity bugs found and fixed along the way: `RenderView::set_configuration` (see Core.2 exit note) and `OverflowBox`/`SizedOverflowBox` silently dropping `alignment` changes on rebuild — `RenderConstrainedOverflowBox`/`RenderSizedOverflowBox` never exposed the `set_alignment` their shared `AligningShiftedBox` component already had, so a widget rebuilt with a new alignment kept its first-mount alignment forever; fixed by adding the missing setters and wiring them into both widgets' `update_render_object`, with a red-then-green regression test for each (confirmed by temporarily reverting the fix). One stale memory note corrected: `SliverGridDelegate` is not behind `experimental-delegates` (only `CustomClipper` still is). Also closed `layout/flow.rs` (46.34%→93.88% region — the last remaining lowest-coverage file): unlike `OverflowBox`, `Flow`'s `update_render_object` already correctly threads both `delegate` and `clip_behavior` on rebuild (a pure coverage gap, not a bug); added direct unit tests for `clip_behavior`'s default/override/rebuild wiring. Also closed `interaction/visibility.rs` (59.26%→72.22% region): `Visibility` was untested anywhere in the suite; added tests for all three `build()` branches against the Flutter oracle (default visible splices the child directly; hidden shows the default/custom replacement; `maintain_state` wraps in `Offstage`, verified against its actual `perform_layout` rather than assumed). Also closed `image/image.rs` (52.87%→90.86% region): added tests for `create_render_object`'s zero-size placeholder on decode failure and `update_render_object`'s "keeps the previous intrinsic size, only clears the painted image" contract on a failed re-resolve (a custom provider that succeeds once then fails, verified via `RenderImage::compute_size` — explicitly public for exactly this kind of layout-sizing test without a full pipeline pass). Also closed `clip/clip_path.rs` (55.56%→96.46% region): like `flow.rs`, `ClipPath`'s `clip_behavior` threading through `create_render_object`/`update_render_object` was already correct (pure coverage gap, not a bug); added direct unit tests for the default/override/rebuild wiring and the "clipper closure is always reinstalled on update since it has no identity to diff" contract. Also closed `interaction/mouse_region.rs` (56.36%→93.33% region): `MouseRegion`'s `cursor`/`opaque`/`behavior` wiring through `create_render_object`/`update_render_object` was already correct (pure coverage gap, not a bug); added direct unit tests verified via `RenderMouseRegion`'s own getters. Current: **80.33% regions / 81.07% functions / 82.69% lines** — closing in on the ≥85% target; full history in the git log for this branch. |
