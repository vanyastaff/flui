...
| N5.2 | ↳ Phase 2 — keyed reconciler completion + `ElementCore` rewiring + `ReconcileEvent` trace stream | ◐ ~90% (audited 2026-06-30) | **Audit done:** `tree/id_reconcile.rs` (1576 LOC) handles keyed reorder/shrink/grow/type-mismatch/deep-teardown — all green. GlobalKey reparent works via the inactive-queue path (ADV-1 case 1, `from_parent: None`). **Only remaining gap:** cross-parent *same-frame Active→Active* reparent (ADV-1 case 2), which `element_tree.rs:1333` documents as **requiring KTD-9's Variable-arity ID storage** — so the tail is KTD-9-coupled, not standalone | Variable-arity reconciliation passes keyed and positional fallback tests (already green); A→A reparent tail tracked under KTD-9 |
