...
| H8 | **flui-app binding singleton → parallel-safe tests** | ⚠ open (assessed 2026-07-07, ship-wave 4) | flui-app + flui-binding | **Assessment:** the `impl_binding_singleton!` process-global (`OnceLock` instance, mirroring Flutter's `WidgetsFlutterBinding.instance`) is deliberate Flutter parity — the singleton itself stays. What forces workspace-wide `--test-threads 1` is *tests sharing that global* (a local `SEMANTICS_TEST_LOCK` mutex already papers over one case). **Fix path:** migrate flui-app tests off the global onto `flui-binding`'s `HeadlessBinding` (built for exactly this — non-singleton deterministic driver); serialize the residual tests that genuinely exercise the singleton behind a shared test-lock; then drop `--test-threads 1` from CI + justfile. Design-reviewed change — needs a dedicated PR with before/after parallel-run evidence. |
