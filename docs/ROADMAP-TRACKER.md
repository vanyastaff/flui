...
> Its remaining edges are the still-missing pan/zoom start/end callback surface
> plus the same hit-test pipeline follow-up as
> `MouseRegion.opaque = false` for `HitTestBehavior::Translucent`
> behind-target behavior.
...
| Input / leaf (`RenderParagraph`, `RenderImage`, `RenderMouseRegion`, `RenderPointerListener`) | ◐ partial | `RenderMouseRegion` and FLUI's `RenderListener`/Flutter `RenderPointerListener` exist + first oracle-verified harness/widget slices 2026-07-01. `RenderParagraph`/`RenderImage` exist; not yet parity-audited. |
