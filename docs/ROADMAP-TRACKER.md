...
| B1.5 | `flui-widgets` coverage ≥ 85% | ⚠ verify | Initial measurement 2026-07-01 (`cargo llvm-cov -p flui-widgets --no-fail-fast`, needed due to one order-dependent flaky test — confirmed pre-existing `flui-app` singleton-state flake per AGENTS.md's Testing Quirks, passes cleanly at `--test-threads 1`): 69.35% regions / 70.16% functions / 72.64% lines. Closed all five 0%-coverage files found in a per-file audit (`interaction/absorb_pointer.rs`, `paint/decorated_box.rs`, `layout/fitted_box.rs`, `scroll/sliver_opacity.rs`, `text/text_field.rs`) to 58-90% each via widget-mount tests (the `common::lay_out`/`Viewport` harness), including one genuinely novel end-to-end test (`AbsorbPointer`'s hit-test absorption blocking a real `GestureDetector`) and one composition test (`TextField`'s real `build()` — every prior test in that file hand-simulated the key handler instead of mounting the widget). Also closed `flex/flex.rs` (50.40%→64.80% region): the plain `Flex` widget itself (only `Row`/`Column`, its fixed-direction wrappers, were tested) and `MainAxisAlignment::End`/`SpaceBetween` (every prior test forced `MainAxisSize::Min`, leaving no free space for alignment to matter) were untested. While investigating `Flex`/`Row`/`Column`'s `update_render_object` (a full struct replace rather than field-by-field mutation), traced `RenderBehavior::on_update` and confirmed it's always immediately followed by `mark_render_needs_layout_and_paint` — no stale-state window, not a bug. Also closed `stack/stack.rs` (50.00%→53.77% region): every prior test used `Positioned` children exclusively, so `Stack`'s own default (`Alignment::TOP_LEFT`, `StackFit::Loose`) and explicit alignment/fit behavior with plain non-positioned children were untested. Also closed `semantics/mod.rs` (37.30%→98.99% region — the largest remaining gap): `Semantics`'s ~28 chained builder methods and the four `SemanticsOptions` bit-flags had no tests at all. Since `create_render_object`/`update_render_object` are plain trait methods needing no pipeline, wrote direct unit tests (no widget-mount harness) chaining every builder method and verifying each property reaches the built `RenderSemanticsAnnotations` via its getters, plus `MergeSemantics`/`ExcludeSemantics` coverage. Also closed `image/provider.rs` (13.95%→94.02% region): under the crate's default feature set (`images`/`network-images` both off), `DirectImageProvider`'s clone-through resolve, the accessors, and `FileImage`'s real I/O feeding into the feature-gated `decode_bytes` fallback had zero tests — only the `images`-feature-gated decode path had a test module, uncompiled in the default coverage run. Also closed `text/text.rs` (49.09%→83.64% region): `max_lines`, `style` (font size), and `direction` were never exercised — added comparative tests (max_lines(1) shorter than unlimited wrapping; larger font_size taller box; Ltr/Rtl identical size) that hold regardless of the font backend, since exact glyph metrics are font-dependent. Also closed `scroll/sliver_list.rs` (36.36%→100% region): `SliverChildBuilderDelegate::new`'s builder-past-item_count contract and its `Debug` impl were never directly tested. Also added `tests/spacer.rs` (52.17%→69.57% region): `Spacer` was untested anywhere in the suite; added flex-splitting and centering tests mirroring `tests/flex_parent_data.rs`'s `Expanded` coverage (a first cross-axis-offset assumption was wrong — verified empirically and corrected before committing). Also closed `scroll/sliver_fixed_extent_list.rs` (51.61%→100% region): `item_extent` threading into actual sizing was already proven via existing `ListView`/`CustomScrollView` mount tests, but `Debug`, `has_children` on an empty list, and `update_render_object`'s own field-level effect (verified via the derived `Debug` output, since `RenderSliverFixedExtentList` has no public getter) had no direct tests. Also closed `scroll/sliver_grid.rs` (51.61%→97.89% region): `Debug`, `has_children`, and `update_render_object`'s grid-delegate replacement had no direct tests (verified via `RenderSliverGrid::grid_delegate()`'s `Debug` output, since the delegate trait has no downcast support). Corrected a stale memory note in the process: `SliverGridDelegate` is NOT behind `experimental-delegates` — only `CustomClipper` remains gated (verified via `crates/flui-rendering/src/delegates/mod.rs`'s own doc comment). Current: **77.47% regions / 79.07% functions / 80.63% lines** — closing in on the ≥85% target. |
