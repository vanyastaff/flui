...
**CONSEQUENCE — Phase 1 is NOT mechanical transcription; it is real `ElementKind` DESIGN work:** decide, per Flutter semantics, the right home for `ErrorElement` and `RootRenderElement` (a new `ElementKind` variant each? refactor them onto the unified `Element<V,A,Behavior>`? a `Special`/`Adapter` variant?), then the render-arity blanket impls. This is a deliberate architecture decision (warrants chief-architect review), exactly the kind that must be *right* for "competitors applaud" — not bashed out under a deadline. **This is the true reason Phase 1 needs a focused design session.** The earlier "pure transcription" framing was corrected by this finding.
