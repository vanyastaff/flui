...
| N5.2 | ↳ Phase 2 — keyed reconciler completion + `ElementCore` rewiring + `ReconcileEvent` trace stream | ◐ in-progress (audited 2026-06-30) | **Production gate now green:** `BuildOwner::build_scope` feeds `build_into_views` into `reconcile_children_by_id`; `production_reconcile_emits.rs::variable_arity_reorder_through_build_scope_preserves_ids_and_emits_parent_id` proves keyed reorder preserves IDs and emits the real parent id through the public rebuild path. Direct slab corpus still covers keyed reorder/shrink/grow/type-mismatch/deep teardown/hash collisions/duplicate keys/S3 permutations. GlobalKey reparent works through the inactive-queue path (ADV-1 case 1, `from_parent: None`). **Remaining gap before full Phase 2 closure:** cross-parent same-frame Active→Active reparent (ADV-1 case 2), still documented in `element_tree.rs` and `global_key_reparent.rs`. | Keyed/positional variable-arity reconciliation is production-locked; close the Active→Active GlobalKey tail before promoting the whole Phase 2 row to `✓ done`. |
| N5.3 | ↳ Phase 3 — `IntoView` surface + `downcast_ref` elimination + derive macros + port-check triggers | ☐ todo (scope corrected 2026-06-30) | **Scope correction:** the original "`downcast_ref` count = 0 in framework code" criterion is misconceived. **Element-side type recovery is already downcast-free** — Phase 1 (`ElementKind`) achieved it (only 3 element-targeting downcasts remain, all in `#[cfg(test)]` against the `LeafView` fixture). The ~63 remaining framework downcasts are **View-side** (`&dyn View` → `V`, inherent to the `View` trait being `dyn`; `dispatch.rs` marks them `PORT-CHECK-OK` and dates removal to **Phase 4+**) plus `ViewKey`-equality. Removing them needs a typed-config / generic-`IntoView` redesign that ripples workspace-wide — large/strategic, not in Phase 3 | Phase 3 = `IntoView` ergonomics + derive macros (element-side downcast goal already met); View-side `downcast_ref` elimination re-scoped to a separate **Phase 4+** typed-config redesign |
