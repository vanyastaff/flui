...
| W4-B1 | Parley with per-realm `FontContext` over a shared collection; text crosses the display list as neutral shaped runs; rasterization on the raster side (swash, per W0-4); `FONT_SYSTEM` leaves the globals allowlist; a door that feeds new faces into the atlas-owned rasterizer; a neutral FLUI-owned blob handle on shaped runs; interned variation coordinates in `GlyphKey`; FLUI's own family resolution; the Parley `complex-scripts` feature decision | L | `flui-painting`, `flui-engine`, `flui-runtime` | glyph oracle tests; `cargo xtask globals` allowlist shorter by one; caret and selection tests over glyph byte ranges; a same-key bitmap-determinism test; a baseline test against today's rounding | Parley drops glyph byte ranges (reported in the market survey) | cosmic-text path behind a flag for one wave | ADR-0092 (supersedes ADR-0077; ADR-0016 and ADR-0059 on acceptance) |
