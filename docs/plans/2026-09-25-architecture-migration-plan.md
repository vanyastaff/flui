...
- **Decisions and their ADRs:** [design/decisions.md](../../design/decisions.md) (ADR-0081 to ADR-0097, all Proposed)
...
| crates.io name availability (`flui-sdk`, `flui-reactive`, `flui-protocol`, `flui-platform-api`, `flui-runtime`, `flui-engine-cpu`) | W1-B2, W2-A1, W2-B1, W3-A1, W3-B3, W6-A3 | checked in the step that creates each crate; nothing reserved ahead |
...
| W0-2 | `ReadScope` and the move of `Signal` behind it | 2 days | `count.get(cx)` compiles unchanged at call sites; `cargo check -p flui-view -p flui-rendering` on the prototype branch | ADR-0085, W3-B1 | an extension trait in the prelude instead of the upcast |
| W0-3 | Open capability seam | 1 day | the 136 `&dyn LifecycleContext` sites still compile; a crate outside the workspace registers a capability and receives it; `Unsupported` is typed | ADR-0084, W6-B1 | keep the closed `LifecycleContext` method list and record why |
...
"until the #1090 field-mask registry lands" (`Cargo.toml:642-645`), and the view crate's comment
...
| W2 | `flui-runtime` extraction: one frame transaction, defined by type; `flui-testing` above the runtime; widgets' inline test modules move to `tests/` | `flui-protocol` extraction; `tools/desktop-mcp` becomes the `flui-mcp` library | TSF implementation; D1b Win32 (callbacks lose `Send`) | B0 |
...
| W1-A1 | `tier = V\|C\|S\|R\|K\|H\|pkg`, `tier-kind` and in-tier order in every `[package.metadata.flui]`; the `workspace` check enforces direction by tier, with `allowed-dev-dependents` for dev cycles (`crates/flui-view` and `flui-testing` today) | M | `tools/xtask`, every manifest, `docs/adr` (the ADR-0078 amendment, W1-A10) | `cargo xtask workspace` 0 findings; `cargo xtask workspace --self-test` **(new flag)** fails on a planted upward edge | the layer model and the tier model disagree during the switch | keep `layer` beside `tier` for one wave, check both | ADR-0081 |
| W1-A2 | "Core names no official crate": `allowed-dependents` generalised by `tier-kind`, reverse direction strict from day one with named, dated exceptions: `flui-testing` dev → `flui-devtools` (`crates/flui-testing/Cargo.toml:106`), `flui-app` optional → `flui-hot-reload` (`crates/flui-app/Cargo.toml:65,108`), the facade's optional and dev edges to `flui-hot-reload` (`Cargo.toml:519,554`) and its optional `flui-material`/`flui-cupertino` edges (`Cargo.toml:525-526`), which are also the only H → `pkg` exceptions to the direction rule (ADR-0081 §1); `flui-cli` (kind `tool`) needs none; forward direction in allowlist mode (Material's core edges and `flui-view/runtime-internals`, the latter removed by W3-A8) | S | `tools/xtask` (`tools/xtask/src/workspace.rs:80,146,248`) | `cargo xtask workspace` green; self-test fails on a planted `flui-widgets → flui-material` optional edge | an exception without an owner lingers | every entry carries its exit step; the allowlist can only shrink | ADR-0081, ADR-0088 |
| W1-A3 | `cargo xtask reach` **(new)** over the resolved `cargo metadata --locked` graph, no target filter, every facade feature combination; K forbid set {flui-platform, winit, android-activity, ndk, windows, objc2-app-kit, objc2-ui-kit, wgpu, flui-engine, flui-app}; `jni`/`windows-sys`/`core-foundation`/bare `objc2` allowlisted with reasons; per-crate `reach-forbid` additions; the three hot-reload `TREE_FACTS` (`tools/xtask/src/tasks/facade.rs:53`) move into it | M | `tools/xtask` | `cargo xtask reach --self-test` fails on a planted edge; `cargo xtask reach` green **with two allowlisted edges**, `flui-interaction → flui-platform` and `flui-widgets → flui-platform` under the `testing` feature (`crates/flui-widgets/Cargo.toml:94`, turned on by the facade's `testing`, `Cargo.toml:592`), both removed by W1-B2 | false positives through optional FFI | allowlist entries with reasons | ADR-0081 |
...
| W1-A5 | Move `realm_dispatch_tests` out of `crates/flui-app/src/app/runner/realm_dispatch.rs` (production is lines 1-1690; the test module starts at :1691) | M (moved lines) | `flui-app` | the file drops off the `file-length` allowlist; `cargo nextest run -p flui-app` green with the same test count | none | revert the move | none |
| W1-A6 | `globals` **(new)**: syn scan of every `static`, `thread_local!` and `OnceLock` outside `#[cfg(test)]`; allowlist seeded by the scan in the same PR (regex estimates do not seed it). Known entries: `APP_RUNTIME` (`crates/flui-app/src/app/runner/host.rs:46`), `AssetRegistry::global` (`crates/flui-assets/src/registry/mod.rs:83`), `TIME_DILATION` (`crates/flui-scheduler/src/config.rs:43`), `REQUEST_REBUILD` (`crates/flui-hot-reload/src/dispatch.rs:24`), `REGISTRY_STACK` (`crates/flui-view/src/key/registry.rs:204`), `NAVIGATOR_COMMAND_TARGETS` (`crates/flui-widgets/src/navigator/navigator.rs:91`), `FONT_SYSTEM` (`crates/flui-painting/src/text_layout/layout.rs:124`); each entry names its exit step | M | `tools/xtask` | `cargo xtask globals --self-test` fails on a planted static; `cargo xtask globals` green | the scan is blind to environment-variable selection such as `FLUI_HEADLESS` | record that limit in ADR-0097; do not claim it | ADR-0097 |
| W1-A7 | `module-dag -p flui-widgets` **(new)**: declared import direction between widgets modules; also pins reactive step 1 later (no `crate::` imports in the reactive module) | M | `tools/xtask`, `flui-widgets` | `cargo xtask module-dag -p flui-widgets --self-test` fails on a planted import; green on main | the declared DAG needs a few moves first | allowlist the edges, dated | none |
| W1-A8 | Phase counters and `cargo xtask perf` **(new)**: writes a baseline, non-blocking in B0, blocking at the B1 exit; its CI job runs where W1-A0 placed it | M | `flui-rendering`, `flui-view`, `flui-testing`, `tools/xtask` | `cargo xtask perf` writes the baseline; `cargo nextest run -p flui-testing perf_` | counters on the hot path | atomic-free fields behind `cfg(debug_assertions)` | none |
...
| W1-B2 | D1a: `flui-platform-api` as a mechanical move of the platform traits with re-exports from `flui-platform`; the one production import below `flui-app` (`crates/flui-interaction/src/text_input.rs:27`) switches over | M | `flui-platform`, new `flui-platform-api`, `flui-interaction`, `flui-widgets` (test harness) | `cargo xtask reach` green with both allowlist entries from W1-A3 removed (a reach fact, not `cargo tree -i`, which errors on an absent package); `cargo xtask check-changed`; `cargo xtask cross-typecheck` | none known | the re-exports stay | ADR-0082 |
...
| Step | What | PR | Crates | Acceptance | Risk | Rollback | ADR |
|---|---|---|---|---|---|---|---|
| W2-A1 | Create `flui-runtime` above `flui-widgets` and move the realm, frame, lanes and semantics host out of `flui-app` (series of 4-6 PRs); in B0, as the owner decided. The GlobalKey lookup under the frame lock and `ElementCore.depth` are fixed in this series, each with a failing test first (§8) | L | `flui-app`, new `flui-runtime`, `flui-widgets` | `cargo xtask test` green; `cargo xtask reach` green with `flui-runtime` in tier K | phase-order regressions | a driver-selection flag for one minor | ADR-0083 |
...
| W2-P2 | D1b Win32: platform callbacks lose `Send` (`crates/flui-platform/src/traits/platform.rs:319` and the registrations after it) | M | `flui-platform`, `flui-app` | `cargo xtask cross-typecheck`; a recorded live run on Windows | not executed in CI | a per-backend adapter that keeps `Send` | ADR-0082 |
...
| W3-B1 | **Reactive step 1**, inside `flui-view`: `Signal::get/with/try_*` take `&dyn ReadScope`, `BuildContext: ReadScope`; `ReadScope` is read-only and never returns `Reactive`; `ExternalBuildScheduler` becomes a one-method `RebuildSink`; for each presentation the realm mints exactly two non-Clone drivers (`ElementDriver`, `RenderDriver`) from that presentation's graph; **the `signals` feature is removed** (depends on W0-10) | M | `flui-view`, the facade, `flui-widgets`, `flui-app`, `flui-testing` manifests | `compile_fail` doctests: neither `ReadScope` nor a cloned handle can call a driver hook; `cargo xtask module-dag -p flui-view` **(new target)** shows zero `crate::` imports in the reactive module | upcast ergonomics at call sites | an extension trait in the prelude | ADR-0085 |
| W3-B2 | **Reactive step 2a**, inside `flui-view`: readers become `Element(ElementId) \| Layout(RenderId) \| Paint(RenderId)` (hard-wired to `ElementId` today); phase guards reject or defer writes during layout and paint, modelled on `WrittenDuringBuild` | M | `flui-view` | a test per guard | the graph's element-specific paths | revert; step 2b waits | ADR-0085 |
| W3-B3 | **Reactive step 2b**: one PR moves the module into `flui-reactive` (tier V, internal, `publish = false` until decided) **together with the first render subscriber**: a render-object field read in paint through `PaintContext: ReadScope` and written outside frame phases. `ScrollPosition` is not the first consumer (it is written in `perform_layout`). The warm-edit measurement that decides folding into `flui-foundation` runs in this PR | M | new `flui-reactive`, `flui-view`, `flui-rendering` | a test that fails without the change and shows the repaint itself, not only the subscription; the warm-edit numbers recorded in ADR-0085 | no render consumer by the end of W3 | the graph stays in `flui-view` and ADR-0085 says so explicitly | ADR-0085 |
...
| W4-P1 | D1b AppKit | M | `flui-platform`, `flui-app` | `cargo xtask cross-typecheck`; a recorded live run on macOS | clippy-only in CI | per-backend `Send` adapter | ADR-0082 |
...
| W5-A1 | `EventCx` slice 1: types, the `callback(\|cx\| ..)` helper (needed for the HRTB error the probe hit), `WriterSource` on `LifecycleContext` | M | `flui-view` (or `flui-reactive` after W3-B3) | `compile_fail` doctests: `&mut Writer` does not escape to `'static`; `build` has no path to a `Writer` | helper ergonomics | stop here; nothing public changed yet | ADR-0086 |
...
| W5-P1 | Linux live evidence; D1b winit | M | `flui-platform` | `docs/evidence/linux.toml`; `cargo xtask cross-typecheck` | none | per-backend adapter | ADR-0082 |
...
| W6-A2 | The raster contract in `flui-layer`: the GPU-free lowering, `RasterBackend`, `PresentDisposition` and a wgpu-free `RasterError`; `RasterOwner` stays in `flui-engine` in H0 | M | `flui-layer`, `flui-engine` | a `reach-forbid` fact that `wgpu` is absent from `flui-layer`'s normal closure; the conformance scenes include a rounded-rectangle and a path clip, so today's bounding-box path clip (`crates/flui-engine/ARCHITECTURE.md:289`) shows up as a named gap or a failure | none known | revert | ADR-0087 |
...
| W6-B1 | Open capability seam: typed registry behind a sealed, object-safe method on `LifecycleContext`; explicit `Application::plugin` registration (no `inventory`/`linkme`); an override hook; the two classes of ADR-0084 §5: core-required capabilities (clipboard and data transfer, text input, accessibility, cursor, window chrome basics) are backend-trait methods, so `PlatformWindow::text_input()` and `accessibility()` lose their `None` defaults (`crates/flui-platform/src/traits/window.rs:333,352`) and the web backend implements them as inert until H1; the built-in clipboard provider from W4-A3 becomes reachable as `cx.capability::<Clipboard>()`; the [AGENTS.md](../../AGENTS.md) "Extending FLUI" row changes in the same PR and states the classification rule | M | `flui-view`, `flui-runtime`, `flui-platform-api`, `flui-platform` | a crate outside the workspace registers a capability; `Unsupported` is typed; a headless fake; a `compile_fail` doctest that a backend without `text_input` does not compile | a backend that returns `Unsupported` for a core capability | revert; core capabilities keep their `Platform` methods either way | ADR-0084 |
...
| W6-P1 | Clipboard live runs on macOS and Linux, dialogs per OS; D1b UIKit and Android | M × 2 | `flui-platform` | `cargo xtask cross-typecheck`; recorded runs | not executed in CI | per-backend adapter | ADR-0082 |
...
- **[S]** `cargo xtask workspace` (tiers) has zero findings. `cargo xtask reach` is green with the
  K forbid set over every facade feature combination, which needs W1-B2. `cargo xtask module-dag
...
- **[R]** The globals, file-length, undocumented-unsafe and multiple-versions allowlists are seeded
  by a scan and only shrink. A perf baseline is recorded (not blocking).
...
|---|---|---|
| GlobalKey lookup can deadlock during a frame: the binding holds `inner.write()` across `build_scope` | `crates/flui-view/src/binding.rs:1241,1297` | W2-A1 (a `Busy` error or an owner-local binding, with a test) |
| `ElementCore.depth` stores the sibling slot | `crates/flui-view/src/element/generic.rs:151` | W2-A1 |
| EditableText branches on the compile-time platform | `crates/flui-widgets/src/text/editable_text.rs:1583` | W4-B2 |
...
code has no bare `unwrap()` left (clippy `unwrap_used` enforces it) and no file above 2000
production lines. Proposed:
...
> the reactive graph stays in flui-view through step 1 and step 2a, and moves to flui-reactive (tier V, internal) in the PR that adds the first render subscriber (ADR-0085)
...
- The warm-edit gain of `flui-reactive`: a line-count estimate that includes test lines (W3-B3
  measures it).
