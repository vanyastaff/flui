...
| crates.io name availability (`flui-sdk`, `flui-reactive`, `flui-protocol`, `flui-platform-api`, `flui-runtime`, `flui-engine-cpu`) | W1-B2a, W2-A1a, W2-B1, W3-A1, W3-B3, W6-A3 | checked in the step that creates each crate; nothing reserved ahead (`flui-runtime`: free on crates.io on 2026-09-26; `flui-sdk`: free on 2026-09-26, so the fallback `flui-package-sdk` is not needed) |
...
| W0-4 | Parley glyph to `GlyphImage` into the existing ADR-0067 atlas, with a stable key (glifo against skrifa) | 1 week | glyph oracle tests green; no process-global font state; the rasterizer runs outside the shaping lock | ADR-0092 (carries ADR-0077 precondition 1), W4-B1 | ADR-0092 stays Proposed; text stays on cosmic-text with `FONT_SYSTEM` on the globals allowlist. **Outcome (2026-09-26):** met on `spike/parley_atlas` (`38af54e25`). swash, driven directly, is the rasterizer: bit-identical to cosmic-text's swash path on 416 glyphs (near-tautological, same scaler); glifo, the independent check, agrees (mean similarity 0.90–0.99) but is 1.26–1.44× slower on outlines and 3.7× on emoji, experimental and vertical-hinting only, so it is not adopted. Parley plus swash is 1.14–2.04× faster than cosmic over 1,000 lines (reviewer's recomputation, not re-run; timing is not part of the gate). The atlas is unmodified except its key type. Keys are stable only while a raster-side registry holds each blob (fontique's shared source cache holds blobs weakly). FLUI adds no lock; fontique locks internally on a local cache miss, shared across realms. Open for W4-B1: a neutral run that does not carry `parley::FontData`, and feeding new faces to the atlas-owned rasterizer. Not verified: wasm32, macOS, Linux, GPU readback, same-key bitmap determinism, 1 px baseline agreement, variable fonts and synthesis, CJK segmentation (`complex-scripts` off), registration after the atlas exists. See ADR-0092 Context |
| W0-5 | Subsecond on Windows, macOS and Android | 1 week | a logic edit keeps state; an edit to a `State` type restarts the realm; no stale TLS or static breaks after a patch; patches reach code behind `Box<dyn ElementBase>` vtables created before the patch | ADR-0094, W5-B1 | keep the dlopen path (and its documented hazards, W0-11) until a later spike. **Outcome (2026-09-26):** failed on Windows (`spike/subsecond`, `51d223fa5`; macOS and Android not run). Stock `dx` cannot patch a FLUI app on Windows x64: its missing-symbol stub `mov rax, imm64; jmp rax` clobbers `__chkstk`'s argument (fix: `jmp [rip+0]`, as on non-Windows). With a patched `dx`, a logic edit keeps state and a `State` size change is caught by a size/alignment/drop/name fingerprint and remounts the root. App-crate statics are zeroed per patch and app-crate thread-locals crash on first access from patched code. Patches reach only calls the hook wraps (6 of 33 build calls per pass), so the hook moves to the element seam. The fail column applies: the dlopen path stays. See ADR-0094 Context and §5 |
| W0-6 | `LayoutCallbackScope` for lazy sliver children | 3 days | a lazy band converges in one pass without a double `RefCell` borrow | ADR-0017 (Revisited); a superseding ADR after a follow-up spike with four conditions | nothing changes; ADR-0083's phase order keeps today's layout↔build fixpoint. **Outcome (2026-09-26):** met only for plain lazy rows and only with R1/R2; not met in general (`spike/layout-callback`, `f7e89e8f6`, `92a47723e`). `LayoutBuilder` rows regressed (mount 60,010 nodes laid out against 355 on `main`), 5 of 55 `.with_mut(` sites are routed so the double borrow is not structurally excluded, the band pass budget is lost, and the scope aliases `&mut` from safe code. The fail column has been applied: ADR-0017 records the result and the four conditions; ADR-0083 keeps between-pass servicing |
...
**Unscheduled, owner decides: the R1/R2 invalidation rules as their own change.** W0-6 measured
two rules alone, without the layout callback (ADR-0017, "Revisited"): R1, a service pass that
...
| W1-A2 | "Core names no official crate": `allowed-dependents` generalised by `tier-kind`, reverse direction strict from day one with named, dated exceptions: `flui-testing` dev → `flui-devtools` (`crates/flui-testing/Cargo.toml:106`), `flui-app` optional → `flui-hot-reload` (`crates/flui-app/Cargo.toml:65,108`), the facade's optional and dev edges to `flui-hot-reload` (`Cargo.toml:553,588`) and its optional `flui-material`/`flui-cupertino` edges (`Cargo.toml:559-560`), which are also the only H → `pkg` exceptions to the direction rule (ADR-0081 §1); `flui-cli` (kind `tool`) needs none; forward direction in allowlist mode (Material's core edges and `flui-view/runtime-internals`, the latter removed by W3-A8). **Outcome (2026-09-26):** landed with W3-A4a as the kind rule in `tools/xtask/src/workspace/tiers.rs` (findings "names official" and "off sdk"): the reverse rule reads every edge kind, optional and dev included, and the forward rule an official package's normal and build edges; both admit a refused edge through the dependent's `edge-exceptions`, so no separate allowlist exists and an unused entry is stale. Seeded: `flui-testing → flui-devtools` (ADR-0088), Cupertino's six and devtools' two internal edges (ADR-0088), hot reload's four (ADR-0094); `flui-view/runtime-internals` is a feature on the `flui-hot-reload → flui-view` edge, excused by that entry. The self-test plants a core dev edge, an off-SDK normal edge in and outside `packages/`, a non-official and an excepted member under `packages/`; the planted optional `flui-widgets → flui-material` case is covered by the normal-edge plant, since cargo reports an optional dependency as a normal one | S | `tools/xtask` (`tools/xtask/src/workspace.rs:119,267,416`) | `cargo xtask workspace` green; self-test fails on a planted `flui-widgets → flui-material` optional edge | an exception without an owner lingers | every entry carries its exit step; the allowlist can only shrink | ADR-0081, ADR-0088 |
...
| W1-B2b | `PlatformWindow` moves to `flui-platform-api` without `as_winit` (removed; nothing calls it) and without `accessibility()`, which moves to a host-side `HostWindow: PlatformWindow` subtrait returned by `Platform::open_window`, `WindowOpen::Ready` and `PendingWindow`. Implemented as two commits, `HostWindow` first, then the move (acceptance pending: Android/iOS cross-typecheck, a wasm32 check and the live Windows run were not yet run); `flui-app` carries the bridge in `PresentationWindow`, built by `runner::presentation_window`. Afterwards the realm core's only production `flui_platform` name is `PlatformAccessibility` (see ADR-0082 §2), which the runtime realm-core step must place first | M | `flui-platform`, `flui-app`, `flui-platform-api` | `cargo xtask cross-typecheck`; a recorded live run on Windows (`cargo xtask device windows-input`) | about 50 `dyn PlatformWindow` sites in `flui-app`, across four backends, two of them clippy-only | revert; W1-B2a stands alone | ADR-0082 |
...
| Step | What | PR | Crates | Acceptance | Risk | Rollback | ADR |
|---|---|---|---|---|---|---|---|
| W3-A1 | `flui-sdk`: a separate Evolving crate, host-free, `0.N` bumped on every train, published in the same run; Stable part as whole-module re-exports under the facade's paths, Evolving part only in named modules (`pipeline`, `hooks`, `gpu`); **train guard** `links = "flui_train"` with a trivial `build.rs` on `flui-foundation`. **Outcome (2026-09-26):** `crates/flui-sdk` (K, evolving, order 6, `0.1.0-dev`) re-exports what Material and Cupertino import outside their tests, measured from source (195 leaf paths from 9 crates): `animation`, `foundation`, `types`, `view`, `widgets` whole; `interaction`, `painting`, `rendering` as subsets at the facade's paths; `pipeline` with 3 Evolving items. The guard is on `flui-foundation`, because the facade does not depend on the SDK. `tests/surface.rs` pins identity with the facade and the list; `two_trains_refuse_to_resolve` in xtask shows the resolver refusing two trains with the repository's `links` value (path packages, not a local registry); `cargo xtask workspace` requires the evolving crate's own `0.N` version and the guard on `flui-foundation` alone; `cargo xtask reach` states `flui-foundation` is in the SDK's and the facade's builds. The rustdoc-JSON count waits on the nightly tooling of W8-A2. ADR-0088 `## Migration` lists the moves | M | new `flui-sdk`, `flui-foundation` | a type-identity test (`flui_sdk::<m>::T` is `flui::<m>::T`) and a pinned surface list; a resolver test (path packages) that two trains fail in the resolver, never with E0308; Evolving surface counted from source, reviewed if above ~30 items (the rustdoc-JSON count follows with W8-A2) | the Evolving surface grows into a second facade | the ceiling in ADR-0088 triggers a review | ADR-0088 |
...
| W3-A4 | **Packages move with `git mv` in the same PR as their switch to the sdk**, one package per PR (W3-A4a..A4d below): into `packages/` as members of the root workspace (one lock, one `cargo metadata`); `tier-kind = "official"`; each package's allowlist entries from W1-A2 removed in its PR; an out-of-tree fixture package (outside `members`, depends only on `flui-sdk` through `[patch]`) uses a custom widget, a theme extension and a plugin | L (4 PRs) | the four packages, `flui-sdk`, `tools/xtask` | per sub-step; `cargo xtask package-check` (W3-A3, which lands first) green; `cargo xtask workspace` with the forward allowlist shorter by that package | the sdk surface turns out larger than measured | the sdk item stays Evolving; the package keeps a dated allowlist entry | ADR-0088 (supersedes ADR-0028 in part) |
| W3-A4a | `flui-material`: its nine internal normal dependencies (`crates/flui-material/Cargo.toml:25-49`) become `flui-sdk = { path, version = "=0.1.0-dev" }` plus `tracing`; imports rewritten by the table in `crates/flui-sdk/ARCHITECTURE.md`; `git mv crates/flui-material packages/flui-material` with the root `members` entry, the facade dependency path and the dev-dependency paths (`../../crates/…`) updated; dev-dependencies on internal crates (`flui-testing`, …) stay (owner question); `allowed-dependents` stays until W3-A5. Material examples stay with the facade until W3-A5, because they need `required-features = ["material"]` (owner question: the row above said they move here). **Outcome (2026-09-26):** done ahead of W3-A3 (`package-check` still waits). The SDK's surface did not change. The `rg` finds nothing in `packages/flui-material/src`; tests name the framework through `flui_sdk::` too, keeping `flui-widgets` (harness), `flui-testing` and `flui-interaction` (key events, `FocusManager`) as dev-dependencies. The derives failed on an SDK-only manifest, so `flui-macros` now resolves `flui-sdk` first. `allowed-dependents` went in this change, replaced by W1-A2's kind rule, which is stricter; Cupertino keeps its list until W3-A4b. Test counts unchanged: 736 listed (495 unit, 241 `material_it`), 39 doctests passed and 1 ignored | M | `flui-material`, root manifest, facade manifest | `rg 'flui_(widgets\|view\|types\|objects\|rendering\|foundation\|animation\|interaction\|scheduler\|painting)::' packages/flui-material/src` finds nothing; `cargo xtask reach` (the `pkg` set already forbids `wgpu`); `cargo xtask check-changed` | an import has no SDK path | add it to the SDK with the pin test updated, Evolving if the facade lacks it | ADR-0088 |
| W3-A4b | `flui-cupertino`: the same (`crates/flui-cupertino/Cargo.toml:17-57`); independent of W3-A4a | M | `flui-cupertino`, root manifest, facade manifest | as W3-A4a for `packages/flui-cupertino` | as W3-A4a | as W3-A4a | ADR-0088 |
| W3-A4c | `flui-devtools`: its imports measured the same way; the SDK gains `hooks` for the observation seam (ADR-0040); the `flui-testing` dev edge to `flui-devtools` goes by moving that test into devtools | M | `flui-devtools`, `flui-sdk`, `flui-testing` | as W3-A4a; the reverse gate has no `flui-testing` exception | `hooks` grows | keep the dev edge as a named exception | ADR-0088, ADR-0040 |
| W3-A4d | `flui-hot-reload`, **together with W3-A6**: needs W3-A8 (`runtime-internals` becomes `__runtime`) first; the `windows` / `android_log-sys` reach exceptions stay until W5-B1 | M | `flui-hot-reload`, `flui-app`, facade | as W3-A4a; W3-A6's acceptance | as W3-A6 | as W3-A6 | ADR-0088, ADR-0094 |
| W3-A5 | **Facade `default = []`** (today `Cargo.toml:719`), no `material`/`cupertino` features (`Cargo.toml:726-727`; the `hot-reload` feature at `:664` already went with W3-A6); `pub use flui_material as material` (`src/lib.rs:148-149`) and the Material half of the prelude removed; a `flui_material::prelude`; the `flui create` counter template adds `flui-material` explicitly, with a CLI test that generates it and runs `cargo check`; `cargo xtask facade-combos` rewritten; the document list comes from `rg 'flui::(material\|cupertino)\|features.*(material\|cupertino)'` over the repository minus `docs/archive`; CHANGELOG migration note; Material examples move to `packages/flui-material/examples`. **Only after W3-A4a and W3-A4b land**, so H0 authors keep a path to both design systems | M | `flui` facade, `flui-material`, `flui-cli`, `tools/xtask`, docs; the workflow's Material build (§9 rows 1-2) already moved by W1-A0 | `cargo xtask facade-combos`; the generated counter builds; `cargo xtask docs-links` | a document still says `flui::material` | the `rg` list is the checklist | ADR-0088 |
...
| W3-A7 | Delete `flui-tree` (Arity/Slot/Depth into `flui-foundation`, the TreeRead/Nav/Write trio becomes inherent methods) and `flui-localizations` (the RTL table into `flui_widgets::localization`) | M + S | `flui-tree`, `flui-localizations`, `flui-foundation`, their users | `cargo xtask workspace`; `cargo xtask check-changed` | none known | revert | ADR-0081 records the crate fate. **Outcome (2026-09-26):** both crates are deleted. `flui-tree`: the arity markers and `IndexedSlot` went to `flui-foundation`; `Depth`, `Slot`, `SlotBuilder`, `SlotIter`, `TreeError` and `ArityError` had no user and were deleted; the trait trio became inherent methods on `LayerTree`, `SemanticsTree` and `RenderTree`. `flui-localizations`: it had no translated strings, so its RTL table and delegate went to `flui_widgets::localization` and nothing went to the packages; the facade keeps an empty `localizations` feature until §9 row 14 |
...
| W4-B1 | Parley with per-realm `FontContext` over a shared collection; text crosses the display list as neutral shaped runs; rasterization on the raster side (swash, per W0-4); `FONT_SYSTEM` leaves the globals allowlist; a door that feeds new faces into the atlas-owned rasterizer; a raster-side font registry that holds every keyed blob; a neutral FLUI-owned blob handle on shaped runs; interned variation coordinates in `GlyphKey`; FLUI's own family resolution; the Parley `complex-scripts` feature decision | L | `flui-painting`, `flui-engine`, `flui-runtime` | glyph oracle tests; `cargo xtask globals` allowlist shorter by one; caret and selection tests over glyph byte ranges; a same-key bitmap-determinism test; a baseline test against today's rounding; a registry test: a prune under a held blob keeps keys equal | Parley drops glyph byte ranges (reported in the market survey) | cosmic-text path behind a flag for one wave | ADR-0092 (supersedes ADR-0077; ADR-0016 and ADR-0059 on acceptance) |
...
| W5-B1 | Subsecond hot reload behind the runtime `DevReloadHook`, after `REQUEST_REBUILD` and `REGISTRY_STACK` leave the globals allowlist; the dlopen path and its three-crate template are deleted only after a later spike passes (W0-5 failed on Windows). Preconditions: the `dx` stub fix (upstream or carried by `flui run`); the no-`static`/`thread_local!` rule for app-crate reloadable code; the structural hash over `View` and `ViewState` types; owner-thread patch application; macOS and Android runs | L | `flui-hot-reload`, `flui-runtime`, `flui-cli` | `flui run --hot` keeps state on Windows and macOS (B1 exit); `cargo xtask globals` allowlist shorter by two | Subsecond on Windows fails with stock `dx` (W0-5) | keep the dlopen path, with W0-11's hazards documented | ADR-0094 |
...
- Subsecond: Windows run (fails with stock dx); macOS and Android unverified (W0-5).
