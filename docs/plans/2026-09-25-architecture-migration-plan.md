...
| W1-A7 | `module-dag -p flui-widgets` **(new)**: declared import direction between widgets modules | M | `tools/xtask`, `flui-widgets` | `cargo xtask module-dag -p flui-widgets --self-test` fails on a planted import; green on main | the declared DAG needs a few moves first | allowlist the edges, dated | none |
...
| W1-A14 | Burn down the `markers` allowlist (`tools/xtask/allowlists/markers.toml`, 92 entries seeded by W1-A4): rewrite each recorded marker as the invariant it stands for, crate by crate, lowering or removing its entry in the same PR | S (series) | every crate with an entry, `tools/xtask` | `cargo xtask markers` green with an empty allowlist | a rewrite loses the reason the label pointed at | revert that crate's PR; its entry comes back at the old count | ADR-0078 |
...
| W3-B1 | **Reactive step 1**: the read contract in `flui_foundation::read_scope` (ADR-0085 §2); `Signal::get/with/try_*` take a generic `&S where S: ReadScope + ?Sized`, `BuildContext: ReadScope`; `ReadScope` is read-only and never returns `Reactive`; subscription only through driver-minted `ReaderSink`s; a wrong-type handle is `SignalError::TypeMismatch`; writes through `SignalWriteExt` in `flui_view::prelude`; `ExternalBuildScheduler` becomes a one-method `RebuildSink`; for each presentation the realm mints exactly two non-Clone drivers (`ElementDriver`, `RenderDriver`) from that presentation's graph, the graph staying in `flui-view`; **the `signals` feature is removed**, with default-build `signals_rebuilds` and idle-frame numbers recorded (depends on W0-10) | M | `flui-foundation`, `flui-view`, `flui-rendering`, the facade, `flui-widgets`, `flui-app`, `flui-testing` manifests | ADR-0085's Verification table: every context shape compiles; a read in `build` through a `flui-testing` mount rebuilds exactly the reader (also `--release`); `TypeMismatch`; `compile_fail` pairs: `Reactive` is not a `ReaderSink`, no driver is `Clone`, `ReadScope` reaches no driver hook | upcast ergonomics at call sites | `&dyn ReadScope` with blanket impls for `&S` and `Box<S>` | ADR-0085 |
