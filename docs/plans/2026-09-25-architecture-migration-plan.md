...
| W1-A3 | `cargo xtask reach` **(new)** over the resolved `cargo metadata --locked` graph, no target filter, every facade feature combination; K forbid set {flui-platform, winit, android-activity, ndk, windows, objc2-app-kit, objc2-ui-kit, wgpu, flui-engine, flui-app}; `jni`/`windows-sys`/`core-foundation`/bare `objc2` allowlisted with reasons; per-crate `reach-forbid` additions; the three hot-reload `TREE_FACTS` (`tools/xtask/src/tasks/facade.rs:53`) move into it | M | `tools/xtask` | `cargo xtask reach --self-test` fails on a planted edge; `cargo xtask reach` green **with five seeded `reach-exceptions` entries**, two of them exiting with ADR-0082: `flui-interaction → flui-platform` and `flui-widgets → flui-platform` under the `testing` feature (`crates/flui-widgets/Cargo.toml:94`, turned on by the facade's `testing`, `Cargo.toml:626`), both removed by W1-B2; the others are `flui-hot-reload → windows` and `→ android_log-sys` (exit ADR-0094) and `flui-engine → wgpu` (a grant from ADR-0081) | false positives through optional FFI | allowlist entries with reasons | ADR-0081 |
...
| W1-B2 | D1a: `flui-platform-api` as a mechanical move of the platform traits with re-exports from `flui-platform`; the one production import below `flui-app` (`crates/flui-interaction/src/text_input.rs:27`) switches over | M | `flui-platform`, new `flui-platform-api`, `flui-interaction`, `flui-widgets` (test harness) | `cargo xtask reach` green with both ADR-0082 entries from W1-A3 removed (a reach fact, not `cargo tree -i`, which errors on an absent package); the stale rule forces both removals, since each entry becomes a finding once its edge is gone; `cargo xtask check-changed`; `cargo xtask cross-typecheck` | none known | the re-exports stay | ADR-0082 |
