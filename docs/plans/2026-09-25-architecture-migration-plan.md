...
| W2-A1c (moved; the wasm32 acceptance waits on CI's `wasm-check` and `wasm-test`) | `ExecutionServices` (formerly `app/execution.rs`, ADR-0047) moves to `flui_runtime::execution`, and the runtime gains `allowed-dependents = ["flui-app"]`; `flui-app` keeps `pub use flui_runtime::execution::{ComputeJob, DeterministicExecutors, HostComputePool, HostExecutors, HostIoPool, IoFuture, SpawnError}` so public paths are unchanged | M moved | `flui-app`, `flui-runtime` | `cargo nextest run -p flui-runtime -p flui-app` with the same test count; the wasm32 `Backend::Sequential` lib tests | tokio enters the runtime's normal graph (native only) | revert the move | ADR-0083, ADR-0047 |
| W2-A1d (moved; acceptance waits on CI's `cross-typecheck`, `wasm-check` and `wasm-test`) | The realm core: `ui_realm/*` (with `attach.rs`), `presentation.rs`, `presentation_forest.rs`, `lifecycle_state.rs`, `frame_failure.rs`, `media_query_root.rs` (`from_window` included: it names only `PlatformWindow`), `bindings/renderer_binding.rs` and `RealmServices`/`next_identity`; the ADR-0048 `catch_unwind` in `draw_frame_entered` moves unchanged, the realm tests move with a headless `FrameSink` (`ScriptedSink`); `UiRealm::enter_for_close` is deleted (redundant after W2-A1a). Moved after three precursors: `PlatformAccessibility` to `flui-semantics` (ADR-0082 §2 amended), `REDACTED_VALUE` to `flui-foundation`, and the realm's own `ReloadTier` | L | `flui-app`, `flui-runtime`, `flui-widgets` | `cargo nextest run -p flui-runtime -p flui-app` with the same test count; `cargo xtask cross-typecheck` | about 250 realm tests and the dev-dependencies they need build for wasm32 for the first time (`flui-runtime` is in the wasm-test set, `flui-app` was not), and the `dead_code` expectations removed for wasm, Android and iOS were not compiled for those targets | revert the move | ADR-0083 |
