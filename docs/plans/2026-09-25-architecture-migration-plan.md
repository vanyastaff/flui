...
| W1-A8 | Phase counters and `cargo xtask perf` **(new)**: writes a baseline, non-blocking in B0, blocking at the B1 exit; its CI job runs where W1-A0 placed it. **Baseline recorded** (`crates/flui-testing/perf/baseline.toml`); `perf --self-test` runs in `checks`; the advisory `perf` job W1-A0 placed (`design/ci.md`) is not in the workflows yet | M | `flui-rendering`, `flui-view`, `flui-testing`, `tools/xtask` | `cargo xtask perf` writes the baseline; `cargo nextest run -p flui-testing perf_` | counters on the hot path | atomic-free fields behind `cfg(debug_assertions)` | none |
