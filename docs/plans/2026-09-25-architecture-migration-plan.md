...
| crates.io name availability (`flui-sdk`, `flui-reactive`, `flui-protocol`, `flui-platform-api`, `flui-runtime`, `flui-engine-cpu`) | W1-B2a, W2-A1, W2-B1, W3-A1, W3-B3, W6-A3 | checked in the step that creates each crate; nothing reserved ahead |
...
| W1-A3 | `cargo xtask reach` **(new)** over the resolved `cargo metadata --locked` graph, no target filter, every facade feature combination; K forbid set {flui-platform, winit, android-activity, ndk, windows, objc2-app-kit, objc2-ui-kit, wgpu, flui-engine, flui-app}; `jni`/`windows-sys`/`core-foundation`/bare `objc2` allowlisted with reasons; per-crate `reach-forbid` additions; the three hot-reload `TREE_FACTS` (`tools/xtask/src/tasks/facade.rs:53`) move into it | M | `tools/xtask` | `cargo xtask reach --self-test` fails on a planted edge; `cargo xtask reach` green **with two allowlisted edges**, `flui-interaction → flui-platform` and `flui-widgets → flui-platform` under the `testing` feature (`crates/flui-widgets/Cargo.toml:94`, turned on by the facade's `testing`, `Cargo.toml:626`), both removed by W1-B2; W1-B2 landed first, so the allowlist starts empty | false positives through optional FFI | allowlist entries with reasons | ADR-0081 |
