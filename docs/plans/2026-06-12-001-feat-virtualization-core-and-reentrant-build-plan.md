...
- **Combined `seek_range` (single shared-prefix descent)** — `query` currently does four independent `offset→index` descents (visible/cache start/end), measured at ~89ns @100k after the U1/P2 descent-reduction (6→4). The four offsets are sorted and, for a real viewport, share almost their entire root-to-leaf path (they diverge only in the last 1–2 levels). A `seek_range` that descends once to the offsets' divergence point and splits there would cut the descent count from `4·log n` toward `~1·log n`, projected to drop `query` to ~25–30ns @100k. Deferred (not done in U1): it is an intricate new tree method whose boundary-rule correctness must be proven against the existing seek; ship it **only with the mid-list proptest + a before/after criterion number**, not on assertion. Recorded here so the win is not forgotten.
