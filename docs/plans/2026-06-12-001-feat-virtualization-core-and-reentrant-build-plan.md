...
> **STATUS: contract SHIPPED** (`148cb3d1`). `ChildLayout<G> { Ready(G) | Scheduled | NoChild | Unwired }` (`#[non_exhaustive]`) + `BoxChildRef { id, size }` (`#[non_exhaustive]`) + `SliverLayoutCtxErased::build_and_layout_box_child(index, constraints, build: FnMut(usize) -> Option<Box<dyn RenderObject<BoxProtocol>>>) -> ChildLayout<BoxChildRef>` (default `Unwired`) + ergonomic wrapper. ARCH-GATE passed: `Ready` carries identity+geometry so a future true-mid-pass backend slots in without a breaking change; hardened by an adversarial review that caught a `Ready(Size)` (identity-less) ossification trap. **The production v1 next-frame scheduler (collect build-requests during layout → drain via the deferred queue once `&mut owner` is free → next-pass build) is DEFERRED INTO U3**, with its consumer — building it bare now (no driver) would be scaffold-ahead-of-consumer. Production contexts return `Unwired` until U3 wires the backend into `Direct` storage.
...
> **U3a (production v1 next-frame backend) SHIPPED** (`f07c33e1`). `build_and_layout_box_child` is now real in the sliver walk: `ErasedSliverLayoutCtx` returns `Ready(BoxChildRef)` for an existing child, parks an absent child's freshly-built object in a `SubtreeBorrows::pending_builds` sink and returns `Scheduled`, or `NoChild` on a declining builder; `layout_dirty_root` drains the sink into `defer_insert_box` after the walk (drop `SubtreeBorrows` first → sound). Unit-tested at the context level; miri-clean on the walk (get_subtree_mut + u20).
>
> **U3b (`RenderSliverListLazy` object) SHIPPED** (`d3b710de`). Adapter, Virtualizer drive, builder closure, dispose-on-scroll-off, anchor-correction policy, `logical_to_slot` map, and `SliverMultiBoxAdaptorParentData` wired.
>
> **U3c (re-entrant child-build contract completion) SHIPPED** (this branch). D1 defect fixed: `PendingBuild` carries `initial_parent_data`; `apply_deferred_mutation` installs it on the fresh `RenderNode` when `parent_data = None`. D2 dispose path fixed: `SliverLayoutCtx<A,P>` Proxy arm now overrides `child_id` and `dispose_box_child`, forwarding to `ErasedSliverLayoutCtx` (previously fell through to the no-op trait defaults). Parent-data seeding in the sliver walk made unconditional (`dyn_clone::clone_box` from `RenderNode.parent_data()`). 4/4 U3c acceptance tests pass (step3, step7, 9a, 9b). Bench: `lazy_sliver/query_band` and `lazy_sliver/frame_settled` added to `benches/virtualizer.rs`.
...
- **Approach**: build the adapter, drive the `Virtualizer`, materialize only the visible+cache band through the U2 contract, feed measured extents back via `set_measured`, **accumulate and apply the signed `AnchorCorrection`**, and dispose on scroll-off via the pluggable hook (Decision 3 — **not** a two-level pool). **Also wires the U2 contract's production v1 next-frame backend** (deferred here from U2 so it lands with this, its driver): the lazy sliver's `perform_layout` collects build-requests into a context-owned sink during the walk; the pipeline drains the sink after `perform_layout_raw` returns (when `&mut owner` is free again) into `defer_insert_box` + a self-`mark_needs_layout`, so the child is built+laid-out a pass later and the next `build_and_layout_box_child` returns `Ready(BoxChildRef)`. The dispose path keys on the `BoxChildRef.id` returned by `Ready`. Cache-region prefetch (build the cache band ahead) hides the one-pass insertion latency. **Correction policy is the consumer's job (the core only emits the signed delta):** *suppress anchor correction while scrolling backward* — applying corrections during an upward scroll is the canonical "items jump while scrolling up" bug — and *reset the correction accumulator on user-initiated scroll*. Keep the eager lists in place; this unit *adds* the lazy one and proves it.
...
- **Combined `seek_range` (single shared-prefix descent) — SHIPPED** (`ExtentTree::seek_sorted`). `query` resolves all four band edges (cache/visible start/end) in one descent that partitions the sorted offsets among children, recursing into each child once — `O(log n + k)` instead of four independent descents. Measured: single-point query (1px window) 89ns → 52ns @100k; realistic window (800px + 250px cache/side, the new `query_window` bench) ~77ns @100k, flat in n, **956×** a naive four-linear-scan baseline. Correctness proven by a `seek_sorted_agrees_with_scalar_seek` proptest (absolute-position invariant vs the scalar `seek_offset`, now a `#[cfg(test)]` reference oracle). Boundary-rule caveat that bit during the build: at an exact item boundary the two f32 summation orders attribute the point to adjacent items, so the test compares `offset_of(idx)+into`, not `into` directly.
