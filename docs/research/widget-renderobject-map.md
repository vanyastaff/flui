...
>
> **`RenderAnimatedOpacity` correction (updated 2026-08-10):** `FadeTransition` and `AnimatedOpacity` now both reuse the persistent `RenderAnimatedOpacity` path. Each widget state owns a `ProxyAnimation`; the private render view injects it once, and animation ticks update alpha/paint without rebuilding the widget subtree. The retained-layer gap remains: FLUI repaints the subtree because it still has no `updateCompositedLayer`-style blend-only path.
>
...
| Widget | Flutter RenderObject | FLUI Status | Arity | Notes |
|--------|---------------------|-------------|-------|-------|
| `FadeTransition` | `RenderAnimatedOpacity` | **Exists** | Single | Persistent `ProxyAnimation`; ticks update the render object without widget rebuilds |
...
| `AnimatedOpacity` | `RenderAnimatedOpacity` | **Exists** | Single | Persistent state-owned `ProxyAnimation`; retargets update the existing render object and ticks avoid widget rebuilds |
