...
>
> **ClipContext consolidation (follow-up complete).** The deferred ClipContext work landed on branch `refactor/clipcontext-consolidation` as an 8-commit batch matching the same atomic-commit-per-finding shape as PR #81. The follow-up went beyond pure deletion: it adopts `flui-painting::ClipContext` as the single canonical trait (matching Flutter's `painting/clip.dart` location), renames `canvas_mut` → `canvas` for Flutter naming parity, drops the `Clip::None` short-circuit to match Flutter's unconditional save/restore semantics, and lands the full `RSuperellipse` clip stack (Canvas op + ClipShape variant + DrawCommand variant + CommandRenderer trait method + dispatch arm) to reach Flutter's 4-method parity. `flui-rendering::context::clip::ClipContext` deleted; `CanvasContext` now implements the painting-crate trait. `flui-painting/ARCHITECTURE.md` lines 37 and 94 are now accurate (the prior "doc-lie" became truth). Commits on the branch:
>
> - `93435c7c` — docs: brainstorm + plan source-of-truth docs.
> - `941f1087` — U1: add `DrawCommand::ClipRSuperellipse` variant + command_ops coverage.
> - `e4b39863` — U2: add `Canvas::clip_rsuperellipse` + `ClipShape::RSuperellipse` (also adds `Copy` derive to `RSuperellipse` to match `RRect`).
> - `d2703703` — U3: add `CommandRenderer::clip_rsuperellipse` trait method + dispatch arm. Default-method falls back to `clip_rrect` against the superellipse's outer rect; real iOS-squircle SDF in the wgpu fragment shader is tracked as follow-up under Step 5 item 14 (`SUPERELLIPSE_CACHE` work).
> - `ca5ebec6` — U4: Flutter-alignment edits on `flui-painting::ClipContext` (rename accessor, drop `Clip::None` short-circuit, add `clip_rsuperellipse_and_paint`).
> - `45e11363` — U5: migrate `CanvasContext` impl + delete `flui-rendering::context::clip` module.
> - `5e4273b5` — U6: align `ARCHITECTURE.md` + `clip_context.rs` rustdoc to post-migration truth.
> - `512bba80` — U7: suppress `expect_used`/`panic` clippy lints on the U2 test module (deferred fixup landed here).
