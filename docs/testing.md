...
```bash
cargo xtask checks                        # fmt, typos, taplo, markdown links (docs-links: lychee, offline), workspace (layers, manifests, test reachability, ADR numbers), toolchain, wgsl, globals (process-global state, ADR-0097), the docs-only allowlist, font assets; builds only xtask
...
```bash
cargo xtask checks --strict                                   # fmt, taplo, typos, markdown links, workspace layers, toolchain, wgsl, globals, ...; a missing tool fails
