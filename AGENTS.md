...
5. **cross-check** — `cargo check -p flui-platform` for `x86_64-pc-windows-gnu` and `x86_64-apple-darwin`. `cargo check` does not link, so the Win32/AppKit backends type-check from Linux; before this job they were only ever compiled by whoever developed on that OS. Local: `just cross-check` (needs `rustup target add` for both)
