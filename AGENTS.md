...
- **Live E2E smoke (`just live-smoke`, CI's `live-smoke` step)** — `tools/live-smoke` drives a REAL windowed demo with REAL X11 input (XTEST) under Xvfb and asserts on captured pixels and the exit code. It is the only executing coverage of the band ABOVE synthetic event dispatch — platform translation, the event-loop wake chain, window-close teardown — each of which shipped broken while every synthetic gesture test stayed green. X11 only (the Wayland teardown path is uncovered); wheel is deliberately unasserted until pointer-scroll dispatch exists.
