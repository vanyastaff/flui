...
- **CI runs nextest fully parallel.** `AppBinding` and `Scheduler` are still process-global singletons, but nextest gives each test its own process; the in-process race under plain `cargo test` is serialized by `SINGLETON_WINDOW_TEST_LOCK`/`SCHEDULER_PHASE_TEST_LOCK` (`crates/flui-app/src/app/runner.rs`). A new test that mutates shared singleton state must take the matching lock. `RenderingFlutterBinding` is no longer singleton-backed (its `impl_binding_singleton!` invocation is deleted) — its tests construct a fresh, independent instance per test instead of sharing a process-wide one, so there is nothing left for a lock to serialize there.
...
5. **Test flake (flui-app singleton)** → a test is mutating process-global `AppBinding`/`Scheduler` state without taking the matching lock (`SINGLETON_WINDOW_TEST_LOCK`/`SCHEDULER_PHASE_TEST_LOCK` in `runner.rs`, or an equivalent guard). Serialize it; do not reach for `--test-threads=1`.
