...
| No process markers (`Cycle N`, `Phase B`, wave and slice labels, `PR-N`, spec task ids, `H`-tracker ids) in comments, doc comments, strings, Markdown text or TOML/YAML/WGSL outside the archival roots | `cargo xtask markers`; allowlist `tools/xtask/allowlists/markers.toml`, exact counts that only shrink |
