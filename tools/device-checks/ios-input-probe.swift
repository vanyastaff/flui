...
// same lesson `cargo xtask device macos-launch-render` records — a window can exist, hold a
...
// `tools/device-checks/check-ios-input.py` reads that file and decides.
...
/// images matched — the same discipline `cargo xtask device macos-launch-render` follows,
