...
on a real window, and runs `tools/device-checks/macos-ax-client.swift` against the
...
Usage: python3 tools/device-checks/check-macos-a11y.py
"""
...
# The build step honours CARGO_TARGET_DIR, so the artifacts are read from the
# same place (a relative value is relative to the repository root, as cargo
# resolves it when run from there).
TARGET_DIR = REPO_ROOT / os.environ.get("CARGO_TARGET_DIR", "target")
BINARY = TARGET_DIR / "release" / "examples" / "a11y_probe"
CLIENT_SOURCE = Path(__file__).resolve().parent / "macos-ax-client.swift"
CLIENT_BINARY = TARGET_DIR / "a11y" / "ax-client"
