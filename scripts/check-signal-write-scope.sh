...
# two arguments. `Reactive::write` at runtime refuses the same thing
# (`SignalError::WrittenDuringBuild`); this scanner is the static half, so the
# refusal is a review-time verdict and not a test-time panic.
#
...
writes='\\.set\\(&?[a-z_][a-z0-9_.()]*,|\\.update\\(&?[a-z_][a-z0-9_.()]*,|\\.set_if_changed\\(|\\.effect\\(|\\.effect_owned_by\\(|\\.computed\\(|\\.computed_owned_by\\('

scan() {
  awk -v guarded="${guarded_fns}" -v toks="${writes}" '
    FNR == 1 { inside = 0; depth = 0; seen_brace = 0; in_closure = 0; closure_base = 0 }
    {
      line = $0
      sub(/\/\/.*$/, "", line)
...
self_test() {
  ...
  local status=0
  echo "self-test: rejected fixture (signal write / memo / effect inside build, layout, paint)"
  if scan "${fixtures}/rejected.rs.fixture" >/dev/null 2>&1; then
    ...
    if [[ "${found}" -ne 7 ]]; then
      echo "  FAIL: expected 7 violations across every write/creation token, got ${found}"
    ...
    for tok in '.set(' '.update(' '.set_if_changed(' '.effect(' '.effect_owned_by(' '.computed(' '.computed_owned_by('; do
  ...
  echo "self-test: accepted fixture (reads in build, Cell::set, writes from init_state / callbacks / effects)"
