...
- **Status:** Proposed. The ADRs this folder relies on (ADR-0081 to ADR-0097) are `Proposed`,
  ...
  the semantics host and the commit epoch; the realm core has not moved yet. ADR-0093: `Routable`,
  `RoutePath`, `Router` and `RouterHandle` are implemented in `flui-widgets`, over an addressed
  Navigator; the derive and the later items of its implementation series are not. The module-DAG gate
