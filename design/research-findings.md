...
| `SignalWrite` applies to the primary presentation | `crates/flui-app/src/app/ui_realm/commands.rs:450-455` calls `self.widgets()`, which is `presentations.primary().widgets()` (`crates/flui-app/src/app/ui_realm/presentations.rs:357-358`), and that accessor is compiled out on Android, iOS and wasm (`:355`) | Confirmed |
