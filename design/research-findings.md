...
| Damage is always full | `crates/flui-app/src/app/raster_lane.rs:291`; `DamageRegion` has only `Full` (`crates/flui-layer/src/scene_snapshot.rs:18-20`) | Confirmed |
...
| M | `SemanticsHost::announce` (`crates/flui-runtime/src/semantics_host.rs:254`) has no caller outside its tests, so live regions (snackbars, validation errors) never reach the OS | Confirmed by grep | [interaction_semantics][m-inter] | Issue |
...
| 12 to 20 Evolving SDK items; plugins depend on about 30 crates; a Win32 edit rebuilds 3 crates | Partial: the SDK holds 3 Evolving items, counted from source (`crates/flui-sdk/ARCHITECTURE.md`); the rest are estimates | Count through rustdoc JSON; `cargo tree -i` | [q2_sdk_stability][q2], [workspace_topology][m-ws] | ADR-0088 |
